...
- [x] R1 — Stabilize the inherited Task 231 worktree. Inventory the existing changes by owner, remove no accepted work, and split them into coherent atomic commits after their already-recorded focused checks are reconciled. Do not start another endurance campaign while this board item is open.
  - Landed accepted bundles: `f51e9273` strict Z.AI character generation, `21f33d80` model-only recovery boundaries, `eeb2f0f0` public-character projection, `fadd9716` Opening Planner tool transport, `a8acd337` coherent rendered-action binding, `9a20f478` Game Master tool transport, `0323670c` Judge tool transport authority, and `4403790f` Narrator tool transport.
  - Game Master qualification passed real `zai-coding-plan/glm-5.3` route, obligation, and possession-transform proposal/review cases without retry, repair, fallback, or provider/model substitution; focused tests passed 78/78.
  - Judge qualification passed observe, contact, move, attempt, freeform, and sanitized recovery cases on real `zai-coding-plan/glm-5.3`; one wait response with an empty required reason passed the authorized same-input second attempt from its safe schema coordinate. Focused tests passed 59/59.
  - Narrator qualification passed real `zai-coding-plan/glm-5.3` packets both with and without an application-owned required reply, preserving reply position after local decode without retry, repair, fallback, or provider/model substitution; focused tests passed 46/46.
  - Main `humanizer`/`deslop` semantic review kept the new provider instructions literal and authority-specific; no player-facing prose, fallback truth, domain-contract weakening, or provider-owned game authority was introduced.
  - R2 entry condition is met: each preserved bundle passed its named isolated qualification and focused checks, landed as a separate atomic commit, and no ungrouped product diff remains. No endurance campaign was used.
- [x] R2 — Lock retry and recovery semantics. Retryable transport, timeout, and safely repairable schema failures receive up to three total attempts on the same admitted turn and selected provider/model; prove no lost context, duplicate mutation, repeated charge-bearing game action, stale write, or second scene. Exhaustion preserves an honest recoverable turn.
  - Player external stages now use at most three durable attempts on the same admitted turn. Judge and Game Master retain their frozen input, provider/model, base versions, idempotency identity, and accepted earlier-stage artifacts; attempt four is rejected and explicit Resume does not start a hidden automatic chain.
  - Narration recovery uses at most three attempts on the same operation, packet, receipts, provider/model, and model construction. It publishes one proper scene only after acceptance and leaves the operation honestly failed after exhaustion; the prior reasoning-mode substitution was removed.
  - Actor Replanner recovery now has the same three-attempt ceiling for repairable contract, provider, and timeout failures. Migration `0055` records the exact job/stage/frame/base-version chain, fresh deadline and consumed predecessor for attempts two and three, rejects attempt four, and fences late prior results.
  - Focused evidence passed: Campaign Play application 22/22, Actor Replanner 20/20, database/migration 21/21, repository/turn service 55/55, backend typecheck and production build. Full turn-runtime completed 93/93 assertions; Vitest then emitted only the known worker-shutdown `onTaskUpdate` timeout after the test file passed.
  - R3 entry condition is met. No endurance campaign or synthetic continuity evidence was used for this owner-bounded retry contract.
- [x] R3 — Qualify supported provider/model transports. Exercise the exact Judge, Game Master, Actor Replanner, Opening Narrator, and turn Narrator contracts outside an endurance campaign; record unsupported schema constructs and adapter behavior. A configuration that fails qualification is not exposed as supported.
  - Campaign Play 1.0 currently qualifies `zai-coding-plan/glm-5.3`. Other configurable provider/model tuples remain general application settings and are not claimed as Campaign Play 1.0-supported until they pass this same stage matrix.
  - The Z.AI OpenAI-compatible chat-completions adapter resolves automatic structured output to strict tool mode. Its provider-facing schemas exclude tuples/`prefixItems`, `oneOf`/`anyOf`, `const`, and nullable union nodes that the endpoint did not reliably honor. Private adapters restore only code-owned handles, indices, authority fields, and explicit empty-string null sentinels, then reparse the unchanged exact domain schema before compilation.
  - Preserved short-run evidence from r229 records exact requested/actual `zai-coding-plan/glm-5.3` identity and valid accepted artifacts for one Opening Narrator, seven turn Narrators, two Actor Replanners, seven Game Master stages, and two Judge stages. The query-only database check returned `integrity_check=ok` and no foreign-key violations. This is stage qualification evidence, not endurance acceptance.
  - Current focused verification passed 225/225 across Judge 59, Game Master 78, Actor Replanner 20, Narrator 46, and Campaign Play application 22 tests. It covers strict local rejection, sanitized correction feedback, exact provider/model retention, three-attempt exhaustion, no fourth call, and no synthetic continuity.
  - R2 changed Actor Replanner and narration recovery accounting, not the accepted first-attempt provider transports. The current focused suites prove that those accepted transports remain the first attempt while attempts two and three reuse the same frozen stage identity and fail closed after exhaustion.
  - R4 entry condition is met: every runtime stage in the declared tuple has a real accepted provider artifact plus current deterministic transport/recovery coverage; no long campaign was launched for qualification.
- [x] R4 — Prove adjacent stage handoffs and reviewed subcalls. Cover Judge -> Game Master -> Rulebook, Actor proposal/review -> settlement, and settled packet -> Narrator, including one corrected schema response and one timeout recovery.
  - Judge correction preserves the admitted turn and frozen frame, records the rejected first artifact, accepts attempt two, calls Game Master once, and commits one Rulebook command/receipt batch. Reopening between accepted Judge, Game Master, and settlement stages repeats neither provider call nor mechanical batch.
  - Game Master timeout recovery reuses the accepted Judge ledger, requested model, and admitted frame, then commits the two-command Rulebook batch once. A separate injected mid-batch fault proves the Rulebook transaction leaves zero partial commands, receipts, or events and recovers to one complete batch.
  - Actor Replanner schema correction records attempt one as interrupted and attempt two as accepted on the same job, frame hash, base world version, provider/model, and durable epoch chain. Its reviewed plan replaces the active plan once; due jobs settle in serial order and an accepted stored plan executes once after reopening.
  - Narrator timeout recovery reuses the same result, packet hash, receipt list, operation, narration identity, and provider/model. Attempt two publishes exactly one proper scene; the committed mechanics snapshot is unchanged and integrity/foreign-key checks remain clean.
  - Focused adjacent-stage verification passed 8/8 named integration cases across turn runtime and Actor Replanner. These checks include one safe schema correction, one external-stage timeout recovery, one Narrator timeout recovery, restart boundaries, transactional rollback, and no duplicate settlement.
  - R5 entry condition is met with no provider fallback, synthetic continuity, long campaign, or player input replay.
- [x] R5 — Run short rendered product journeys. Cover new-world and provenance-clone setup, opening, freeform and suggested actions, movement, contact, waiting, autonomous consequence, save/reload, and an exhausted-recovery UI without hidden fallback.
  - The provenance-clone journey `short-glm53-clone-r5-20260817` completed eight coherently bound UI actions spanning suggested and freeform input, observation, movement, contact, waiting, and an autonomous actor consequence. Every settled action had one completed turn/result, one complete `model_accepted` narration operation, one proper scene, and no continuity source. Reload after action three restored the same projection and enabled controls.
  - The same clone journey exercised successful bounded recovery in place, including a Narrator acceptance on attempt three. A separate exact action used the same configured `zai-coding-plan/glm-5.3` tuple against an intentionally unreachable endpoint and exhausted exactly three Game Master attempts; the rendered page retained the prior scene, disabled new choices, and exposed `Resume` for the interrupted turn without fallback, continuity, result, operation, scene, or duplicate input.
  - A fresh `Saltglass Crossing` world was created and accepted through the normal UI with 10 locations, 20 directed routes, 10 generated actors, and 6 pressures. `short-glm53-newworld-r5-20260817` then imported canonical Brina once, completed setup and a model-authored Opening, accepted one rendered contact choice, committed one result and `model_accepted` proper scene, and showed Guildspeaker Anselm Vos leaving for Harbor Bench Steps as an autonomous consequence.
  - The generated-world reload matched at `8e86d204af1652b83538c773094bcde4f0a63b150b5952cd0f7ed3bf67d40b38`; the same narration, five enabled controls, world version 13, and runtime revision 41 survived. Query-only reconciliation returned `integrity_check=ok`, no foreign-key violations, one distinct completed player turn/result/operation/scene, exact requested/actual GLM 5.3 stages, and zero synthetic continuity.
  - The live evidence harness now initializes the ordinary first Campaign Play state for `worldSource=generated` through the existing state repository before freezing eligibility. Focused coverage passed, while the full file retained only its three known seeded-replay Opening interruptions. R6 entry is met without using a long campaign.
- [x] R6 — Run one twenty-to-thirty-action diagnostic journey. Every accepted defect gets a focused regression and its own atomic commit; repeat R2-R5 as affected. Do not promote the diagnostic itself into a sixty-action acceptance attempt.
  - The preserved `diagnostic-20-glm53-black-rain-r6n-20260817` finalized successfully from a separate task-owned copy at `output/playtests/campaign-play/diagnostic-20-glm53-black-rain-r6n-20260817-token-ceiling-copy`: 20/20 completed `model_accepted` actions, `forbiddenContinuity=[]`, 9 schedules, 4 plans, 5 lazy `null` plan IDs, reload proofs at actions 1, 10, and 20, required checkpoints, inventory integrity, and unchanged source hashes. Replay hash is `25aba767b7bc0318f4e2c9175c5719c4acbf7662551481dc04c24b9191516c22`. The bundle reports frozen ceilings of 500000 input and 32768 output with truthful aggregate actuals of 396001 input and 34292 output; canonical validation passed with `promotionEligible=true`. R6 is complete and the R7 entry condition is met; R7 was not launched.
  - The frozen `diagnostic-20-glm53-black-rain-r6i-20260817` journey completed and coherently bound actions 1–11 with model-authored scenes and zero continuity. Action 12 then exposed a semantic contract defect: the admitted route-bound action required both travel and delivery of trunk 4, while the accepted Game Master batch committed travel plus a vague scene event and silently dropped the delivery outcome.
  - Game Master semantic review now receives every proposed world-event outcome plus the authenticated resolution, rejects `player_intent_unfulfilled` when any material task, target, tool, delivery, contact, inspection, or exclusion is dropped, and passes only that safe coordinate into the existing bounded same-turn recovery. It adds no code-authored outcome, fallback, provider/model switch, attempt, or persistence path.
  - Main `humanizer`/`deslop` review kept the new provider instructions literal, outcome-specific, and non-narrative. Focused Game Master verification passed 79/79; Campaign Play application and turn runtime completed 118/118 assertions before the known Vitest worker-shutdown tail; backend typecheck/build and diff checks passed.
  - The fresh `diagnostic-20-glm53-black-rain-r6j-20260817` journey reached a ready model-authored Opening, then froze on action 1 after mechanics completed but all three Narrator attempts failed. Attempts 2 and 3 repeated the same intent index in the provider-facing action-selection array; the operation failed with no proper scene or browser binding.
  - Narrator tool mode now exposes one required code-keyed selection field per available intent instead of accepting model-authored intent indexes. The model chooses each field at most once, code restores the canonical index and required-reply position, and the unchanged packet schema remains authoritative for count, detail, grounding, and scene semantics. Duplicate and out-of-range intent indexes are therefore unrepresentable rather than repaired after generation; no fallback, extra attempt, provider/model change, code-authored wording, or persistence change was added.
  - Main `humanizer`/`deslop` review kept the transport instructions literal and machine-facing, with no player-visible prose change. Narrator verification passed 46/46; Campaign Play Narrator, application, and turn runtime completed 164/164 assertions before the known Vitest worker-shutdown tail; backend typecheck/build and diff checks passed.
  - The fresh `diagnostic-20-glm53-black-rain-r6k-20260817` journey then completed and coherently bound actions 1-8 with `model_accepted` scenes and zero continuity. Its action-8 ready scene exposed the next gameplay defect before another click: the model rewrote the frozen optional contact intent as a report of a second crate that the player had not moved, so the offered choice asserted a future action as already complete.
  - Optional suggested actions are now complete application-owned intents. Narrator may select which frozen intents to publish but cannot write, revise, or complete their wording; code publishes the exact frozen label. Only the application-owned required reply in an active exchange retains model-authored detail. Historical stored prefix/detail labels remain readable without permitting new model-owned optional wording.
  - The code-owned observe intent now carries the complete truthful label `Examine the immediate situation`, preserving its deterministic certified route without recovering hidden semantics from Narrator prose. Main `humanizer`/`deslop` review found the machine-facing instruction literal and the player-facing label direct, with no fallback or synthetic truth.
  - Focused verification passed Narrator 46/46, contracts 36/36, visibility 7/7, and application 24/24. Turn runtime passed 94/94 assertions before the known Vitest worker-shutdown tail; backend typecheck/build and diff checks passed.
  - The fresh `diagnostic-20-glm53-black-rain-r6l-20260817` journey then completed and coherently bound two actions with `model_accepted` proper scenes and zero continuity. Action 3 completed its mechanics but froze before a proper scene: all three Narrator tool calls returned HTTP 200 from the exact configured GLM 5.3 tuple and were rejected only because `requiredReplyDetail` exceeded the hard-coded 80-character transport limit.
  - Required-reply detail now uses the actual remaining player-facing label capacity: the shared 240-character action-label limit minus the exact code-owned actor/action prefix for the frozen intent. The packet-specific native and tool schemas enforce that boundary, the generic proposal envelope no longer rejects a detail that the final label can represent, and the unchanged final narration schema still rejects overflow. No text is truncated, repaired, defaulted, or replaced; no fallback, extra attempt, provider/model change, or persistence path was added.
  - The exact boundary regression accepts a reply whose final prefixed label is exactly 240 characters and rejects the next character. Narrator passed 46/46; Narrator, application, and turn runtime passed 164/164 assertions before the known Vitest worker-shutdown tail; backend typecheck/build and diff checks passed. No player-facing prose or provider instruction changed, so semantic review found no wording delta to humanize or deslop.
- [x] R7 — Run two fresh sixty-action UI campaigns as the final 1.0 playtest: one new accepted world and one provenance clone. Bounded successful retries are allowed and audited; lost state, duplicate input/effect, synthetic continuity, hidden provider/model substitution, or unrecoverable turn failure rejects the lane.
  - Campaign World opening-readiness repair passed one fresh short qualification (Build `b4e04aab-9a2f-49c8-a0f3-1ba16f6f112c`, Accept once, prepare eligible before character creation); Main humanizer/deslop review found the cast/connections instructions literal, authority-specific, and non-narrative; R7 remains unlaunched.
  - R7g character intake repair now projects only the accepted world's complete premise, DNA, and accepted `worldSummary` into the character ingestion context under the existing 16,384-byte limit; `researchSummary` and `sourceReferences` remain stored upstream as research/provenance and are excluded from the derived character-generation projection. Focused character-service regressions cover imported and generated paths, exact semantic preservation/canonicalization, oversized research/provenance isolation, mismatched or unaccepted worlds, and semantic-only overflow; the focused suite passed 43/43.
  - Semantic review verdict: player-character synthesis should consume the accepted playable world's premise, DNA, and worldSummary. `researchSummary` and `sourceReferences` are upstream generation evidence, not player-character instructions; excluding them from the downstream prompt removes provenance noise without changing player-facing copy or model instructions.
  - Qualification `qualification-character-world-context-r7g-20260818` used the supported provenance clone `character-context-r7g-target-20260818` of frozen source `6cf1065a-b2c0-4784-ae5c-4b43b6f5b1c7`; source and target accepted content hash `7af709e6b9502f66c393663a7fedd7570834b7c231b5d489700165a7dd593ff9` and source digest `bd46f493db2832f29929b555d2bc4ce533bc590ead68757d1e496566838b3f02` matched. One original built UI tab prepared once with `eligible=true`, `setupPhase=character_required`, `turns=0`, and `openedAt=null`; canonical Brina was imported once with the configured GLM 5.3 tuple, produced a valid rich draft with Continue enabled, Continue was clicked once, and the target persisted exactly one character before the rendered flow advanced to `opening_required` (Choose your arrival). No setup choice, Begin, opening, or player action was taken. Query-only source and target checks returned `integrity_check=ok` with no foreign-key violations; frozen source `state.db` SHA-256 remained `2fb9e76bc8b8bb820a77e21342abe7b079f687749503315e13c0cf95106675e0`, target `state.db` SHA-256 was `58aff0459b2c2d4de920726feb32d05993afad44a8c9a6cc0f3305809735a87b`. Backend typecheck/build, frontend production builds, and direct route checks passed; adjacent fixture suites remain verification-blocked by their pre-existing missing playable-scene topology. R7 remains unchecked and no endurance lane started.
  - Repair qualification `qualification-narrator-selection-glm53-20260818` used one fresh `black-rain-passage-pristine-54df81f3` provenance clone. Focused Narrator verification passed 46/46; Main accepted the fixed-cardinality Narrator repair on those focused checks plus the built UI journey through one model-authored Opening and action 1, which settled and bound one `model_accepted` proper scene with zero continuity. Action 2 was clicked exactly once and stopped at the independent Game Master null-handle boundary after all three exact `zai-coding-plan/glm-5.3` attempts returned `model_contract_invalid`; no proper scene settled, action 3/reload/finalize were not attempted, and no retry or replay occurred. R7 remains unchecked.
  - Repair qualification `qualification-game-master-null-sentinel-glm53-20260818` used one fresh supported provenance clone of `release-60-glm53-new-world-r7g-20260818` / `6cf1065a-b2c0-4784-ae5c-4b43b6f5b1c7`; acceptedContentHash `7af709e6b9502f66c393663a7fedd7570834b7c231b5d489700165a7dd593ff9`, sourceDigest `bd46f493db2832f29929b555d2bc4ce533bc590ead68757d1e496566838b3f02`, source `state.db` SHA-256 `2fb9e76bc8b8bb820a77e21342abe7b079f687749503315e13c0cf95106675e0`, and canonical Brina SHA-256 `4b48de7a32df6a6be5a91ab12f09bb87926b40d5940348c7581e298eaa12b60a` matched; query-only integrity/FK checks were clean. One built Browser tab prepared once, imported Brina once, continued/setup/began once, reached a model-authored Opening, and captured/signed/authorized/clicked exactly one enabled observe suggestion (`Examine the immediate situation`). One player turn completed with accepted requested/actual `zai-coding-plan/glm-5.3` Game Master, whose decoded actorless `record_world_event` persisted `performingActorId: null`; the model_accepted narration operation completed with a new proper scene, coherent binding, enabled controls, zero duplicate/forbidden continuity, integrity `ok`, and zero foreign-key violations. The same page reloaded once with public-state/replay/checkpoint hashes identical (`matches=true`); finalization reported one completed action and `promotionEligible=true`. Focused Game Master tests passed 81/81, backend typecheck/build and frontend production build passed; adjacent turn-runtime/application suites remain unavailable at the pre-existing invalid Campaign World topology, frontend lint retains its unrelated forge hook error, and GitNexus private-seam impact remains unresolved/UNKNOWN at indexed `93228f5`. R7 remains unchecked and no endurance lane started.
  - Game Master recovery accumulator correction qualification `game-master-boundary-20260819-correction` completed one fresh template-backed built action after prepare/import/setup/Opening: the first Game Master attempt was rejected and attempt two accepted on the same `zai-coding-plan/glm-5.3` tuple, one `model_accepted` scene settled and bound, the page reloaded with `matches=true`, finalization reported `promotionEligible=true`, and the query-only SQLite check returned `integrity_check=ok` with zero foreign-key violations. The direct new-possession provider qualification remains the accepted proof for the unchanged schema/provider path. Main humanizer/deslop verdict: the recovery sentence is literal provider-facing text with no filler, no player-facing copy, and no product-meaning change. R7 remains unchecked and no endurance lane started.
  - Narrator recovery-contract qualification `qualification-narrator-recovery-20260819` used one fresh clean-start materialization of the supported provenance source `090421a7-187c-4c4a-b152-5676fb505226` (`current-pristine-090421a7`, acceptedContentHash `08b3f0390dba43864ac252a1c70421743d9960b8a0d48985aaff0bdc7e84a6dc`; source state/config hashes matched). One built Browser tab prepared once, imported canonical Brina once, completed setup/Opening once, and captured/signed/authorized/clicked exactly one enabled suggested action. The action completed with requested/actual Narrator `zai-coding-plan/glm-5.3` strict-object attempt 1, one `model_accepted` operation, one proper scene, coherent binding, enabled controls, and formal action-1 same-page reload proof `matches=true` (public/replay/checkpoint hash `bfc2f2d9ba4d71d40fe7d6e9928f95c930aa8682ab67daa9bb2a3c71e5195a9b`); finalization reported one action and `promotionEligible=true`. Query-only SQLite reconciliation returned `integrity_check=ok`, empty `foreign_key_check`, one distinct player turn/result/operation/proper-scene binding, unchanged acceptedContentHash, and cleared active operation recovery feedback; failed-attempt audit durability remains covered by the deterministic close/reopen test. Main humanizer/deslop verdict: the recovery sentence is literal provider-facing text with no filler, no player-facing copy, and no product-meaning change. This is a confirmed contract/durability gap repair, not a claim about frozen R7n action 53. R7 remains unchecked and no endurance lane started.
  - Repair qualification `qualification-action-impossible-binder-20260819` used a task-owned copy of the frozen R7r action-6 session/database: the provider-free bind accepted the completed `action_impossible` Judge `impossible/no_effect` result with zero commands/receipts/world events, one complete `model_accepted` proper scene, one settlement and browser ledger row; pointer-loss restart and repeat were idempotent, a conflicting digest failed closed, copied/source `state.db` main bytes and signed/capture/click/render identities stayed unchanged, and query-only integrity/FK checks were clean. R7 remains unchecked; the frozen source was untouched.
  - Narrator `invalid_structured_tool_call` recovery repair qualification `qualification-narrator-invalid-structured-20260819` injected one bounded first-attempt rejection, then accepted the exact requested/actual `zai-coding-plan/glm-5.3` retry with the fixed provider-extraction instruction and no fallback, repair, or substitution; a fresh built one-action Browser journey prepared/imported/setup/began once, settled and bound one `model_accepted` proper scene, reloaded once with `matches=true`, and finalized with `promotionEligible=true`. Query-only SQLite returned `integrity_check=ok`, empty foreign-key violations, one completed player turn/result/operation/proper scene, and no duplicate turn or narration identities. R7 remains unchecked and no endurance lane started.
  - Unchecked R7 recovery-contract correction (2026-08-19): attempts 1-3 are the automatic Game Master recovery episode; after exhaustion the exact interrupted turn remains `retryEligible=true` for one explicit player Resume, which creates exactly one same-turn recovery call with automatic chaining disabled. If that explicit call fails, no automatic attempt follows. R7 acceptance lanes freeze instead of invoking Resume after a boundary. R7 remains unchecked and no endurance lane started.
  - Unchecked R7 repair note (2026-08-19): Game Master strict tool transport now exposes effect-order kind strings and bounded application-owned performer keys; private decode restores chronological partition rows and canonical performer handles with exact coverage and sentinel checks. Focused Game Master verification passed 90/90; backend typecheck/build, frontend production build, and the fresh built qualification `gm-transport-qual-20260819` passed one imported/setup/Opening flow, one exact signed DOM action, one `model_accepted` proper scene, one matching reload, and query-only SQLite integrity/FK reconciliation. Provider metadata retained requested/actual `zai-coding-plan/glm-5.3` strict-object identity; existing bounded Game Master recovery accepted attempt two without provider switch, text fallback, or raw-body retention. The approved provider sentence is literal mechanical contract text and needed no humanizer/deslop rewrite. Adjacent suites completed their assertions but retained the known Vitest worker-shutdown `onTaskUpdate` verification tail. R7 remains unchecked and no endurance lane started.
  - Final new-world acceptance `release-60-glm53-fresh-world-post-spoken-plan-20260823` ran on clean commit `28cd04b51f5e65d3033e257427a9928de611452e` in campaign `2d402315-7537-446e-be8f-22ad42116771`. It completed 60/60 player actions through the built UI with checkpoints and same-page reloads at actions 1, 5, 10, 20, 30, 40, 50, and 60; replay hash `cb8a527e4c7b87f44eafbdbdf712751373bd3785bd5b303ba3fa663e6dbc9012`; `promotionEligible=true`; zero hard failures or interrupted player actions; full receipt/runtime/turn-event coverage; and query-only SQLite integrity/FK checks clean.
  - Final provenance-clone acceptance `release-60-glm53-provenance-post-spoken-plan-20260823` ran on the same clean commit in campaign `e5e41b51-d60f-44e2-90c6-202dae12a74f`, bound to package `black-rain-passage-current-migrated-v2`. It completed 60/60 player actions through the built UI with the same checkpoint/reload schedule; replay hash `77af10358145e5d900f3c4c7fa72d7bec7630609469c6d7cca4212151d7a636c`; `promotionEligible=true`; zero hard failures or interrupted player actions; full receipt/runtime/turn-event coverage; clean provenance; and query-only SQLite integrity/FK checks clean. Both final campaigns kept requested/actual `zai-coding-plan/glm-5.3` without hidden substitution, duplicate input, synthetic continuity, or Resume.
  - Bounded Main release review sampled the opening and actions 1, 5, 10, 20, 30, 40, 50, and 60 from both durable scene streams. The scenes remained grounded and internally coherent. Always selecting the ordinal-zero suggestion produced a repetitive, low-movement route in both campaigns; this is recorded as a content-quality advisory rather than a release hard stop because it introduced no contradiction, hidden-state leak, duplicate mutation, persistence failure, or blocked player input, and the required movement, waiting, freeform, and autonomous-consequence journeys passed at the earlier validation levels. Humanizer/deslop review kept the release prose direct and factual.
- [x] R8 — Perform the bounded 1.0 release review, documentation, packaging, and handoff. Release when R1-R7 pass.
  - WorldForge 1.0 release review reconciled the visible application version and package owners at `1.0.0`, updated the English and Russian release documentation, and preserved the supported Campaign Play provider/model claim. The focused application-shell suite passed 7/7, root typecheck passed, all shared/frontend/backend package suites passed, the offline Campaign Play suite passed 35/35, the production build passed, and the built Settings surface rendered `v1.0.0`. The two final sixty-action campaigns above remain the release journey evidence. Publish as the atomic `release: ship WorldForge 1.0` commit and annotated `v1.0.0` tag; the post-release reliability soak remains non-blocking.
...
Chronology live proof r04:

- Clean r04 reused the same frozen Lowwater Ledger world and Lenna. The signed UI action answered Vassara, travelled unconditionally over the open route, and called to the crews at Winch-Shaft Station. No persuasion condition could split the requested follow-through into another turn.
- Game Master committed Vassara's origin response, Lenna's canonical movement, and the destination call. World time advanced 40 minutes. The rendered state placed Lenna at Winch-Shaft Station with Dessek Holvar Brunne visible there.
- The repaired `What changed` list renders Vassara's Debt-Ward response first and Lenna's route, arrival, and unanswered call second. The public API returns the same causal sequence. The live turn therefore closes the missing multi-event chronology proof.
- Narration is a human prose `PASS`: the mineral grit, cold updrafts, taut cables, and fading echo make the destination legible without inventing an answer. The scene is less dramatic than the strongest reference sample because the call receives silence, but its next choices offer concrete investigation or contact with Dessek.
- The action took 274,653 ms. Interaction pace remains a product risk, and repeated one-exchange deferral would make contact feel inert at this latency. These findings do not invalidate the chronology proof.
- A full backend restart preserved normalized public-state hash `2d046afe00bbe4e8af23b148ae2ad55db24611080ee7c46cbe19ad84c75cb004`. The finalized bundle `output/playtests/campaign-play/chronological-consequences-glm52-lowwater-ledger-fb2c7074-r04` validates with 1/1 signed action, replay hash `a538f68169c457fb169ec70f7d41a1e29b57e813fbe89ad9d805dc5389926fa9`, complete coverage, zero hard failures, and `promotionEligible=true`.
- Humanizer/deslop review found the note direct and specific. Standalone smoke additions remain `0`.

Current replacement Lane A r06, sitting 1:

- Clean r06 reused accepted Lowwater Ledger provenance `fb2c7074e89df263b0bf308f0d47aadcd5f246b6832c8274506c8f1dc26855d9` and Lenna through the normal player contract. No world or character generation ran. Opening and ten manually signed actions completed through the rendered Play UI on Z.AI Coding Plan `glm-5.2`, with no player-action resubmission, fallback, repair, provider switch, model switch, or imposed model timeout.
- Lenna followed a self-chosen clinic-supply problem from Debt-Ward Tenements to Tide Shrine Sanctuary and Winch-Shaft Station. Kaiyu limited gathering to loose moss, the returning tide and Lenna's existing knee brace bounded the result at one bundle, and the persisted item left player inventory only when Lenna handed it to Vassara.
- The sitting produced legible autonomous motion. A rope blockade and watch-roster existed at the station before Lenna arrived; Ostane travelled behind her; Vassara independently left the clinic because Dessek had asked her to tend families holding the line. Vassara then connected the blockade to the clinic's lung-wort shortage without making Lenna the cause of either problem.
- Human verdict is `CONTINUE_LANE_AFTER_BREAK`, not long-play acceptance. Scores are comprehension `5/5`, prose readability `4/5`, agency `5/5`, world aliveness `4/5`, and desire to continue `4/5`. Advisories are a retroactively explained Kaiyu-presence seam, one awkward repeated-name choice label, overly complete Dessek monologues, a near-verbatim repeated closing line at action 10, duplicated consequence/narration presentation, and multi-minute interaction latency.
- A full backend restart after action 10 preserved normalized public-state hash `dedd91dfded254e71a1c554dbf7501eb5d8321aee321acdafb4ddc60f98adcad` and replay hash `905e12ccdbd859205f7260a4301694e245a227371fb3fc17b0a4a7997b5e307f`. The reloaded UI restored Winch-Shaft Station, the same visible cast, possessions, last narration, and choices. Browser console and network-error captures are empty. Sitting 2 must begin with unaided recall before rereading.
- Humanizer/deslop review kept observed mechanics, player interpretation, and taste judgments separate. The note records concrete defects without turning one coherent sitting into proof of sixty-action quality. Standalone smoke additions remain `0`.

Lane A r06-r09 game-loop repair gate:

- r06 action 11 completed the player's intended inspection of the Winch-Shaft chalk roster. Action 12 then committed Lenna's time advance and canonical move to Warden Tollhouse before an actor replanning job interrupted. The public packet remained on the prior Winch-Shaft scene, so r06 was frozen without Resume. The rejected model object was not persisted; evidence proves a successful GLM 5.2 transport followed by `model_contract_invalid`, but not which output field failed.
- Actor Replanner had accepted arbitrary bounded strings for active-goal and target handles even though post-provider compilation required exact frame membership. Commit `40e7af36` derives both handle sets from the frozen actor frame and applies them to the top-level intent and every step intent. Existing compiler checks remain as defense in depth.
- Clean r08 stopped earlier at Judge. The signed suggested wait reached one provider-valid GLM 5.2 response, then failed exactly at the frozen-choice equality check: the schema still allowed the model to change an already selected action's kind or targets. r08 was frozen without Resume.
- Commit `37f1d129` binds suggested-action kind, ordered targets, and movement route directly in the native Judge schema. The player already owns those fields by choosing the button; the model still owns feasibility, elapsed time, uncertainty, and result bounds. No retry, repair, fallback, provider switch, result rewrite, compatibility path, or smoke suite was added.
- Focused validation passes `26/26` across Judge and Actor Replanner contracts, and backend typecheck passes. GitNexus impact and change detection continued to return `Transport closed`; manual caller and changed-file review bound the two patches to Judge request compilation and Actor Replanner proposal compilation.
- Clean r09 reused the same accepted Lowwater Ledger template and Lenna without world or character generation. One signed UI wait advanced world time by twenty minutes. Judge, Game Master, Actor Replanner, and Narrator each accepted one GLM 5.2 attempt. Four scheduled actor jobs settled; Novice Niruko's exhausted plan was replaced by one accepted `plan_retry`, while Vassara's retry remained bounded and deferred. Both Opening and player turn completed with no interrupted stage or Resume.
- A full backend restart after action 1 preserved exact public-state hash `3b9a98cd71f34caaaf3858058f482f97f451dae6a8a949545b59bd214c92c5da`. SQLite integrity is `ok`, foreign-key check is empty, and logs contain no warning or error. This is a targeted live `PASS` for the repaired contracts, not sixty-action acceptance.
- The rendered wait result is understandable and useful: twenty minutes pass, families remain audible behind doors, measured rounds approach, and a sealed delinquency list becomes inspectable. Prose and presentation advisories remain. The consequence sentence begins with the awkward fragment `Twenty minutes pressed still`, the same Narrator paragraph renders twice, and Thessha is already listed in `Present here` while patrol footsteps approach without a clearly identified source. Taste scores are comprehension `4/5`, prose readability `4/5`, agency `4/5`, world aliveness `4/5`, and desire to continue `4/5`.
- Humanizer/deslop review keeps the observed failures, mechanical proof, and taste judgment separate. r06 and r08 remain diagnostic; r09 is the active replacement lane. Standalone smoke additions remain `0`.

Lane A r09 ambient-contact hard stop and Judge repair:

- r09 completed seven further signed manual actions after the repaired wait, for eight completed player actions total. Lenna followed the delinquency writ into Thessha's quota conflict, inspected broken ward lamps, discovered that her chimney panes did not fit, met Dessek after his autonomous arrival, and tested a lit household door. The rulebook denied incompatible glass instead of granting material, Dessek denied authorship of the repairs instead of accepting the player's premise, and Vassara independently left for Tide Shrine Sanctuary.
- The visible consequence for Dessek's arrival incorrectly said he `left for Debt-Ward Tenements` while the cast and Narrator correctly showed that he had arrived. The household remained anonymous but reacted coherently by dimming its light and refusing to open. The ninth signed action offered bounded repair help through the closed door. Its single GLM 5.2 Judge response passed provider/schema validation, then failed at the actionable-contact postcondition because no named visible actor represented the established occupant. r09 is frozen at 8 completed actions; action 9 is unbound and was not resumed.
- Judge now permits an actionable `contact` to target the exact current location when the recipient is an unnamed or collective presence established by the source moment or a cited observation. Empty targets still fail, hidden handles still fail, and the prompt forbids inventing an actor, redirecting speech to another visible person, or treating delivery as evidence of identity, trust, knowledge, compliance, or reply.
- Focused Judge verification passes `18/18` and backend typecheck passes. GitNexus impact remained unavailable with `Transport closed`; manual caller review bounds the change to Judge prompt/compilation in the player-action interpretation path. No retry, repair, fallback, compatibility path, result rewrite, provider switch, or standalone smoke test was added.
- Humanizer/deslop review kept the added instruction mechanical and singular: it names the ambient-recipient representation and authority limit without prescribing prose or repeating style guidance. A fresh pristine clone is required; r09 remains diagnostic and will not be resumed.

Lane A r10 ambient-contact live proof:

- Fresh r10 reused the accepted Lowwater Ledger template and Lenna through the normal player API. No world or character model generation ran. Opening and two signed manual actions completed through the Play UI with one accepted GLM 5.2 attempt per stage.
- After Lenna inspected the stripped doorframes, a full backend restart preserved exact public-state hash `bef4e46099cda02896acdd324be70ca5233135fcc3866b7b4afcb291dcbce885`. She then addressed the unnamed families behind the dark doors while explicitly excluding Vassara and Thessha as recipients.
- Judge preserved `contact`, targeted only the exact current `Debt-Ward Tenements` location, cited the established scene, and stated that delivery succeeded while any reply remained GM authority. It invented no actor handle, trust, identity, knowledge, or compliance. GM chose silence, and Narrator rendered the unanswered call without exposing a hidden resident.
- SQLite integrity is `ok`, foreign-key check is empty, and post-restart logs contain no warning or error. The repaired contract is a live `PASS`; r10 remains the active long-play lane. Humanizer/deslop review keeps the mechanical proof separate from the taste finding that a second silent corridor action is plausible but provides little forward information. Standalone smoke additions remain `0`.

Lane A r10 qualitative continuation and evidence stop:

- Eight player actions remained correctly signed and bound. Lenna learned the lease-count rule, offered bounded repair help, followed Vassara's lead to the Tide Shrine, and negotiated Noro's privacy boundary without forcing access to the concealed cable glow. The shrine offered useful lamp work and a two-tide deadline; Noro directed Lenna toward households that could choose whether to open their doors.
- The prose stayed coherent and spatially grounded. Advisories are the repeated misspelling `Thesssha`, one redundant handoff sentence, and two consecutive silent-door turns that slowed the ward scene. No remote actor was pulled into the shrine and Lenna's three possessions survived travel.
- The ninth game turn completed, but its evidence bind failed because the operator signed the visible option letter `d` instead of the public opaque choice handle. The chosen text matched the durable action; the formal handle did not. r10 is frozen at eight eligible actions. The pending file was not rewritten, the turn was not resumed, and the database was not rolled back.

Lane A r11 opening stop and r12 replacement:

- r11 reused the same pristine template and Lenna draft. Its single open-choice Opening Planner call ended after `230362 ms` with `model_contract_invalid`: GLM supplied an exposure trigger outside the native enum. Text fallback was disabled, no retry or Resume ran, and r11 remains frozen before a completed opening.
- r12 again reused the template and draft, with explicit `Outsider / Just arrived / Following a lead` conditions. Opening Planner accepted attempt 1 in `207269 ms`; Narrator accepted attempt 1 in `52888 ms`. The opening connected Vassara's debt to seized illumination tokens while keeping the clinic shortage separate.
- Action 1 asked Thessha about the seizures. Judge (`22179 ms`), Game Master (`44311 ms`), and Narrator (`68202 ms`) accepted attempt 1. Thessha exposed the three-cycle rule and Warden quota authority, retained her local sympathy, and withheld the remaining household count. A full backend restart restored exact normalized public hash `f0f34f31d4371ef657c245a13983766df568775ca0361becdfb4f6bf7c3dca00`. r12 is the active lane.
- Humanizer/deslop review kept the note chronological and evidence-bound. It distinguishes prose judgment, operator error, model contract failure, and verified recovery without inflating any of them into long-play acceptance. No standalone smoke test, retry, fallback, or compatibility path was added.

Lane A r12 actions 2-6 and destination-contact repair:

- Five further actions were submitted manually through the rendered Play UI. Actions 2 and 3 stayed at the ward but became repetitive: Thessha repeated quota context, and inspecting the salves reused opening props without producing a new decision. Actions 4 and 5 recovered the thread. Vassara named the human cost of missing medicine, explained that the last storm-supply bundle had stopped at Winch-Shaft Station because Remane could not dive, and made lamp work a concrete bargaining resource.
- All first five actions are signed and bound. Action 5 is the strongest prose beat in this sitting: it gives Lenna a specific missing bundle, a destination, a blocked worker, and leverage without resolving the shortage. The next suggested destinations did not include Winch-Shaft despite that explicit lead, so contextual choice generation remains a formative defect.
- Action 6 was the signed freeform attempt to travel to Winch-Shaft with mending tools and ask its operators where the bundle was stopped. Its single GLM 5.2 Judge call returned a native-schema-valid object after `91840 ms`, then failed semantic compilation with `model_contract_invalid`. Game Master never ran and no world mutation occurred. r12 is frozen at five eligible actions without Resume.
- The source packet already carried the exact visible route and destination, and the prior public moment established unnamed shaft operators. Judge's frame discarded the route-to-destination binding, while its contact postcondition allowed only a visible named actor or the current location. This made the valid compound `travel + contact` intent unrepresentable.
- Judge now receives exact visible route-to-destination bindings. For an established unnamed or collective recipient, contact without movement still targets the current location; contact after movement must target the selected visible route's exact destination. The rule authorizes only delivery into the established scene and does not create an actor, identity, trust, knowledge, compliance, or reply.
- GitNexus is available again. Impact is LOW for Judge `prompt` and `compile`; the large runtime factory is reported CRITICAL because it is a graph hub, but its only d=1 callers are the default runtime factory and seeded replay, and its signature is unchanged. The schema constant is duplicated in the index as `Const` and `Function`; the real function node has no incoming dependency edge. Focused Judge/runtime verification passes `55/55`, and backend typecheck passes.
- The default application-factory tests also pass `5/5`. The older seeded replay suite remains unavailable as regression evidence: both cases interrupt in the fixture Opening Planner before Judge runs, with `model_contract_invalid` and no accepted artifact. That pre-existing fixture drift is outside this destination-contact repair and does not block its fresh product proof.
- Humanizer/deslop review kept the new instruction mechanical and authority-bounded. It does not prescribe narrative prose or introduce repair, retry, fallback, compatibility behavior, provider switching, or a standalone smoke suite. A fresh pristine clone is required for product proof; frozen r12 remains diagnostic only.

Lane A r13 destination-contact live proof:

- Fresh r13 reused accepted Lowwater Ledger provenance `fb2c7074e89df263b0bf308f0d47aadcd5f246b6832c8274506c8f1dc26855d9` and reconstructed Lenna through the normal player endpoint. No world or character model generation ran. Opening used explicit `Outsider / Just arrived / Following a lead` conditions and completed through the rendered Play UI.
- The opening is a human content `PASS`: Vassara already owns the unpaid chimney conflict, Thessha is nearby with seizure writs, and both visible routes remain optional. The exact Narrator paragraph renders twice, so presentation remains `REVISE` even though the prose itself is coherent.
- Action 1 asked Vassara who handles illumination tokens at Winch-Shaft. She named Dessek and his people, explained that the shaft keeps its own count, and warned that the trade would meet resistance. The answer establishes a destination presence and a bounded motive without granting tokens or a quest solution.
- The signed action 2 then travelled to Winch-Shaft Station with the mending tools and called to the shaft crew with a repair-for-tokens offer. Judge accepted one GLM 5.2 native-schema attempt in `40818 ms`. Its exact normalized contact targets are the Winch-Shaft destination location and Lenna's tools, with the visible route in `movementRouteHandle`; the contested bargain rolled a setback.
- Game Master accepted one attempt in `34124 ms`. Its chronological batch advanced sixty minutes, moved Lenna over the canonical route, then recorded the destination scene. Rulebook applied the time and movement receipts. Narrator accepted one attempt in `96340 ms`. The rendered UI places Lenna and Dessek at Winch-Shaft, leaves the tools in inventory, gives the crew a cold refusal rather than invented trust or a trade, and offers a direct Dessek follow-up.
- Independent world activity remains legible: fresh knots show Dessek reinforcing the rope barricade while remote Niruko and Thessha events remain outside Lenna's scene. The refusal and barricade preserve a local conflict that did not wait for the player. Human taste scores are comprehension `4/5`, prose readability `3/5`, agency `4/5`, world aliveness `4/5`, and desire to continue `4/5`.
- Prose/presentation advisories remain concrete. Narrator wrote the grammatical error `Workers glance your way, looks flat`, and the UI duplicates each displayed Narrator beat. These do not invalidate spatial or authority proof, but they prevent a clean prose-quality verdict.
- All eight model stages across Opening and both actions used Z.AI Coding Plan `glm-5.2`, strict native objects, attempt 1, with zero errors, retry, repair, fallback, provider switch, or model switch. SQLite integrity is `ok`, foreign-key check is empty, and the Play tab reports no console warning or error.
- A full backend restart and UI reload preserved normalized public-state hash `4e36f37be8248a739602d4294cea9b12e9e9669101328d15c50f84f10eac8180`. The destination-contact repair is therefore live-proven. r13 remains active at two signed actions and may continue as the replacement long-play lane.
- Post-commit GitNexus rebuild was attempted with all `13565` embeddings preserved. It failed inside GitNexus on a duplicate `CodeEmbedding` primary key for `ownActionsFromRows`; pre-commit query, impact, and staged change detection had already succeeded, so no claim is made that the post-commit index is fresh.
- Humanizer/deslop review keeps mechanical proof, state evidence, prose taste, and the separate UI defect distinct. No backend prose rewrite, parser, compatibility path, retry, fallback, provider switch, or standalone smoke test was added.

Lane A r13 continuation and action-8 model stop:

- r13 continued to seven signed manual actions after the targeted proof. Dessek explained the independently maintained blockade, refused to spend the watch's own tokens, and left individual households free to trade for repairs. His first explanation is overly comprehensive and repeats most of the location premise; his narrower response to Lenna's non-Compact offer is stronger because it preserves both the blockade and household agency.
- The signed household approach crossed the rope with Dessek's permission, found many dark shelters, repaired one existing cracked lamp, and earned exactly one persisted illumination token. Other families kept their scarce tokens and watched rather than turning into a queue of customers. This is a human agency and bounded-reward `PASS`.
- Lenna returned to Debt-Ward. The travel result preserved the token but ignored the explicit `look for Vassara` clause even though the cast listed Vassara as present; its suggestions also omitted Vassara. This is a contextual intent/handoff advisory, not a spatial contradiction.
- The next signed action transferred the one persisted token to visible Vassara. Rulebook removed it from Lenna's inventory. Vassara counted it as partial payment, refused to call the larger debt settled, and did not invent a remaining amount. The original conflict therefore changed without disappearing.
- Player interest remained positive through action 7. The craft loop produced one concrete success, connected a family-scale need to the blockade, and returned value to the opening creditor. Advisories are repeated beat duplication, the earlier grammatical error, Dessek's exposition length, and the missing Vassara handoff after return.
- Action 8 was the signed suggested observation `Examine the corridor bench and leather satchel`, bound to opaque handle `choice_bb69b1e4f13c42334845d0b4`. One GLM 5.2 Judge call returned after `25265 ms` with native schema outcome `invalid`; no rejected artifact was persisted. Game Master never ran, no mutation occurred, action 8 is unbound, and r13 is frozen at seven eligible actions without Resume.
- Because the rejected object is unavailable and the selected-action schema already fixes kind, ordered targets, and movement route, this single failure does not justify guessing another contract repair. The next lane uses a fresh pristine clone; no retry, repair, fallback, provider switch, model switch, rollback, or standalone smoke test is introduced.
- Humanizer/deslop review distinguishes the successful seven-action gameplay evidence from the unrecoverable model object and does not turn an unavailable field into a speculative code defect.

Lane A r14, first ten-action sitting:

- Fresh r14 reused the accepted Lowwater Ledger template and reconstructed Lenna through the normal player endpoint. No world or character model generation ran. Opening and ten manually signed actions completed through the rendered Play UI on Z.AI Coding Plan `glm-5.2`.
- The opening gave Lenna a meaningful moral choice: Vassara had two debt-payment tokens ready but needed the same tokens for sulfur-lung salves. Lenna left the medicine money with Vassara and asked for work instead. The world accepted the sacrifice without inventing a reward, then pointed toward Dessek as one possible lead.
- The early ward investigation is mixed. Thessha said ten households were exposed and claimed to name affected rows, but neither the visible consequence nor Narrator preserved any row identifiers. A follow-up trace action again produced only abstract markers. Knocking at a shuttered door plausibly received silence, but the two-turn writ thread did not provide the actionable information its prose claimed existed.
- Lenna travelled to Winch-Shaft, where Dessek denied that paid crews were operating and refused to turn the blockade into a convenient job. He named halved quotas, collectors, and the need for bodies on the line. Lenna then travelled to Warden Tollhouse rather than joining or breaking the line immediately. The route, destinations, cast, and inventory remained canonical throughout.
- Tollhouse play is the strongest part of the sitting. Fresh brine drag-marks and blue-green cable fibers existed before Lenna arrived. Ostane first gave a practiced partial answer, physically guarded the evidence, and made Lenna choose whether to press. Across two follow-ups she disclosed abnormal light-taking threads, increasing brightness after storms, weakening core fibers, and the economic cost of a Compact inspection. She remembered Dessek's line and revised her stance only slightly after Lenna truthfully explained how she arrived. The exchange has resistance, subtext, local stakes, and a world problem that does not exist for the player's convenience.
- Human verdict is `CONTINUE_LANE_AFTER_BREAK`, not long-play acceptance. Scores are comprehension `4/5`, prose readability `4/5`, agency `4/5`, world aliveness `4/5`, and desire to continue `4/5`. The main content defect is the missing writ identifiers. The persistent presentation defect is exact duplicate rendering of Narrator beats. One contextual label, `Ask Ostane Grafton about Ostane's plans`, is mechanically awkward, and several Dessek/Thessha answers remain more expository than conversational.
- All 35 model stages used `glm-5.2`, strict native objects, and attempt 1. There were zero rejected schemas, stage errors, retries, repairs, fallbacks, provider switches, or model switches. Aggregate recorded model time was `1543986 ms`; the reliable output does not make the multi-minute interaction pace acceptable.
- SQLite integrity is `ok`, foreign-key check is empty, and the database contains one completed Opening plus ten completed player actions. A full backend restart and rendered UI reload restored exact public-state hash `acd818d9cc81411d3b0df3966a65b6267c71c20dfe7d6c6b0d2d62ee768d47a4`. The reloaded UI preserved Warden Tollhouse, Ostane, Lenna's three possessions, the latest exchange, and the same four next actions. Browser diagnostics contain only development info/log entries and no warning or error; restart stderr is empty.
- Humanizer/deslop review keeps technical proof, content gaps, presentation defects, and taste judgments separate. r14 is the active replacement lane at ten eligible actions. No standalone smoke test, compatibility path, backend-authored prose, retry, fallback, or provider switch was added.

Lane A r14 actions 11-13 and Game Master summary-bound repair:

- Action 11 inspected the exposed filaments and stayed within visual evidence: irregular blue-green threads crossed the cable lay, displaced strands, and left the core visibly thinner. The result could not identify cause or provenance. Ostane's prior physical guard disappeared without an explicit reaction, so the observation is useful but the immediate interaction continuity is `REVISE`.
- Action 12 explicitly stepped back without touching the cable and asked for Ostane's own plan. Ostane recognized the retreat, stopped guarding the bundle, connected the danger to her twice-daily crossing and the cost of a closed span, and asked Lenna for craft evidence rather than surrendering the problem. This is a human content `PASS`: the NPC keeps authorship, local stakes, caution, and a reason to involve the player's established trade.
- Action 13 was the signed suggested tool test. Judge accepted one GLM 5.2 attempt, preserved the exact choice, rolled a strong success, and bounded the attempt to the visible cable and existing mending tools. Game Master then returned a native object whose first effect summary exceeded the existing 1200-character schema limit. The stage interrupted with `model_contract_invalid`; no Game Master artifact or world mutation was persisted, the action is unbound, and r14 is frozen at 12 eligible actions without Resume.
- The 1200-character limit remains the typed event contract. Game Master prompt now states the existing 1200-character event/possession limit and the shorter condition/relation/goal limit, and tells the model to return only the committed result without planning, reasoning, or repeated support. Backend does not truncate, rewrite, retry, repair, or fall back.
- GitNexus reports LOW impact for the private Game Master prompt, with `plan()` as its only direct caller. Focused Game Master tests pass `26/26` and backend typecheck passes. Humanizer/deslop review kept the new line as a direct format contract rather than narrative style coaching. The repair requires a fresh pristine clone; r14 remains diagnostic and will not resume.

Lane A r15, first ten-action checkpoint:

- Fresh r15 reused the accepted Lowwater Ledger template and reconstructed Lenna through the normal player endpoint. No world or character model generation ran. Opening and ten manually signed actions completed through the rendered Play UI on Z.AI Coding Plan `glm-5.2`.
- The opening established a useful conflict: Vassara had spent her remaining tokens on sulfur-lung salves while Thessha carried fresh seizure writs for ten households. Lenna learned that token locks would freeze household trade and credit before later eviction. Thessha remained an official with limited authority rather than becoming an ally or quest dispenser.
- Lenna inspected and repaired exactly six cracked corridor panels with her existing tools. The world granted no payment. Thessha explained that uncommissioned work credits the ward ledger only through a stamped work order and that the duty clerk does not answer to her. This is a bounded mechanics `PASS`: useful labor matters, but goodwill does not bypass the institution or erase weeks of household debt.
- Actions 9 and 10 expose a repeatable world-memory defect. The game says Lenna read and retained the corridor designation and six panel numbers, then says she recited them one by one for Thessha to verify, but neither visible consequence nor Narrator supplies a single identifier. The prose asserts committed concrete facts while keeping them entirely offscreen, matching the earlier missing-writ-identifier failure.
- Presentation remains `REVISE`. A visual browser pass shows one Narrator paragraph; the apparent second copy in the DOM snapshot is the intentional `sr-only` live region, so earlier duplicate-rendering observations do not describe the sighted UI. The real layout defect is the sticky `Your move` dock covering the middle of the NarrationDock at a 1270 by 720 viewport. `Thesssha` also appears in visible prose despite the canonical `Thessha`, and Vassara left the visible cast during the longer repair without an observable departure beat.
- Human verdict is `CONTINUE_AFTER_REPAIR`, not long-play acceptance. Scores are comprehension `4/5`, prose readability `3/5`, agency `4/5`, world aliveness `3/5`, and desire to continue `3/5`. The strongest beat is the refusal to convert unpaid repairs into easy credit; the weakest is the two-turn claim that exact identifiers were read and verified without ever exposing them.
- All 34 model stages used `glm-5.2`, strict native objects, and attempt 1: one Opening Planner, eleven Narrator calls, ten Judge calls, ten Game Master calls, and two Actor Replanner calls. Every stage was accepted with a valid schema. The fresh lane therefore live-proves the Game Master summary-bound repair without retry, repair, fallback, provider switch, or model switch.
- SQLite integrity is `ok`, foreign-key check is empty, and the database contains one completed Opening plus ten completed player actions. A full backend restart and actual browser reload preserved exact public-state hash `eba110f62b882936a2e06267767fc22a3332ca0bbd69ea59f9cd17841e4bd180`. Browser console, network-error capture, and restart stderr are empty.
- No standalone smoke test or compatibility path was added. The next implementation slice should stop the action dock from obscuring narration and make newly observed identifiers explicit and durable before continuing r15 beyond action 10. Humanizer/deslop review kept mechanical proof, visible defects, and taste scores separate.

Lane A r15 post-checkpoint repair and action-11 product proof:

- The Play layout now keeps NarrationDock at the top of its desktop grid track and caps it above the fixed action dock, with overflow confined to the narration card. At `1280x720`, the live narration occupies `y=100..424` and the action dock begins at `y=434.6`; choices and freeform input remain usable. The narrow layout explicitly removes the desktop height cap.
- The action-9 and action-10 artifacts isolate the missing-identifier defect to Game Master. Judge authorized the observations, but Game Master committed only that six identifiers had been read; Narrator accurately inherited the empty summary. Game Master's prompt now requires every newly revealed, recorded, communicated, or verified in-world value to appear in the committed summary. It forbids claiming a value was read or confirmed while omitting it, and preserves established values exactly. Backend does not generate, truncate, parse, or rewrite prose.
- GitNexus reports LOW impact for the private Game Master `prompt()` function, with `plan()` as its only d=1 caller. Focused Game Master verification passes `26/26`. Humanizer/deslop review kept the new rule mechanical and direct. No retry, repair, fallback, provider switch, compatibility path, or standalone smoke test was added.
- One manual freeform action through the rendered Play UI asked Thessha to dictate the exact markings while Lenna copied them. Judge (`22662 ms`), Game Master (`49694 ms`), and Narrator (`58018 ms`) each accepted one GLM 5.2 strict native object on attempt 1. Game Master committed corridor `W-4-9` and panels `091` through `096`; the visible consequence enumerates all six and Narrator preserves the values. This is a live `PASS` for concrete-fact materialization.
- Prose remains `REVISE`: `Thesssha` still appears, and the same designation shifts from `W-four-dash-nine` to `W-four-nine`. These are model consistency defects, not grounds for backend string replacement. Browser warning/error capture is empty, SQLite integrity is `ok`, and foreign-key check returns no rows.
- The operator submitted action 11 before staging its live-session decision. The durable turn is valid product evidence but cannot be signed or bound retroactively. r15 therefore remains at ten eligible actions; action 11 does not count toward long-play evidence. No rollback, Resume, or evidence rewrite was used.

Lane A r16, first five-action sitting:

- Fresh r16 reused accepted Lowwater Ledger provenance `fb2c7074e89df263b0bf308f0d47aadcd5f246b6832c8274506c8f1dc26855d9` and reconstructed Lenna through the normal player endpoint. No world or character generation ran. The opening and five manually signed actions completed through the rendered Play UI on Z.AI Coding Plan `glm-5.2`.
- Lenna offered Vassara fair repair work without taking medicine or light. She inspected a waxed hairline crack, failed the first low-heat seal, scraped away the ruined wax, and sealed the bare fracture on the second attempt. The failed action changed the object and required a grounded recovery step; success did not invent payment, trust, or a larger solution.
- Vassara then left for Winch-Shaft Station on a canonical route while red-wax writs and a small locked iron box became visible near Thessha. The next choices support four distinct intentions: inspect the box, continue ward repairs, follow Vassara, or question Thessha. Human interest remains positive because the world movement opens a new decision instead of repeating the repair.
- Human verdict is `CONTINUE_LANE`, not long-play acceptance. Scores are comprehension `4/5`, prose readability `4/5`, agency `4/5`, world aliveness `4/5`, and desire to continue `4/5`. The limitation is that four actions stayed on one small object and Vassara did not visibly react before departing. The next sitting must show whether her movement and the official trace matter.
- All 18 persisted stages used `zai-coding-plan` `glm-5.2`, strict native objects, and attempt 1. Every stage was accepted without retry, repair, fallback, provider switch, or model switch. Opening planning took `237516 ms`; the slowest later stage was the action-5 Actor Replanner at `101235 ms`.
- SQLite integrity is `ok`, foreign-key check is empty, and event sequences are contiguous from `1` through `212`. A full backend restart after action 1 preserved exact normalized public-state hash `87b96ba3421e1c25d9e47401effdaa4439c62f31f7157e708a0bd634a62c2ed1`. Browser warning/error capture is empty.
- r16 remains active at five of sixty eligible actions. The memory to carry into the next sitting, recorded before rereading, is: Lenna repaired Vassara's jar after one failed pass; Vassara then left for Winch-Shaft; red-wax papers and an iron box appeared near Thessha. Humanizer/deslop review kept human taste, observed defects, and technical proof distinct.
- GitNexus structural indexing is available again at commit `d6922e3`: `13823` nodes, `36844` edges, `790` clusters, and `300` execution flows. Embedding rebuild remains disabled after GitNexus produced duplicate primary keys for distinct symbol records; structural query, context, impact, and change detection are usable.

Lane A r16, actions 6-10 and restart checkpoint:

- Thessha turned the red-wax trace into concrete play: ten exposed families, a three-cycle seizure threshold, and personal pressure to meet the quota. Her first answer is too complete, but the follow-up is concise and exact. Remane Scoth lives on the next landing, is four cycles behind, and has a cracked-seal writ.
- Remane did not appear when Lenna followed Thessha. Thessha knocked, delivered the writ, and continued her own route. Lenna offered help through the door without claiming authority or demanding entry; the household stayed silent. This preserves refusal and keeps both the official and resident from becoming player-owned quest machinery.
- Action 10 moved Lenna to Winch-Shaft Station and advanced time from `00:44` to `01:14`. The destination had visible aftermath from `00:34`, before the player arrived. Vassara had travelled there independently, Dessek was present, and the rope barricade, charcoal tallies, and clipped `chimney` and `sulfur` exchange created a local problem without a full exposition dump.
- Human verdict remains `CONTINUE_LANE`, not long-play acceptance. Scores are comprehension `4/5`, prose readability `4/5`, agency `4/5`, world aliveness `5/5`, and desire to continue `4/5`. The strongest evidence is the three-turn sequence of closed-door silence, Thessha's continued route, and a destination already changed before Lenna arrived. The action-6 answer remains the main prose advisory because it explains most of the seizure system at once.
- The database contains one completed Opening and ten completed player actions. All 33 model stages used `zai-coding-plan` `glm-5.2`, strict native objects, and attempt 1. Every stage was accepted without retry, repair, fallback, provider switch, model switch, interruption, or Resume.
- SQLite integrity is `ok`, foreign-key check is empty, and runtime event sequences are contiguous from `1` through `365`. The action-10 full backend restart preserved normalized public-state hash `c6c961db6efd5037cc49c30320b71f05fb0a679640eba190ab47c0bca4dca521`. The reloaded Play UI restored the destination, cast, possessions, aftermath, arrival, narration, and choices. Browser console and network-error captures are empty.
- r16 remains active at ten of sixty eligible actions. Humanizer/deslop review kept technical proof, observed play, taste judgment, and the prose advisory separate.
- GitNexus reindexed HEAD `cf6a9630` to `13812` nodes, `36833` edges, `790` clusters, and `300` execution flows. Structural `query`, exact `context`, upstream `impact`, and `detect_changes` all work. The overview resource still reports its previous cached snapshot, while the repository registry and working tools use the refreshed graph.

Lane A r16, actions 11-20 and restart checkpoint:

- Dessek's first chimney explanation is too complete and quest-shaped. Vassara's follow-up is better because it separates observed symptoms from what the repair can accomplish, though it is still dense.
- Actions 13 through 17 are a human content `PASS`. The game materialized exact crate contents, required preparation, preserved a canonical route back to the flue, and exposed a specific seam. Wet compound failed under the downward draft and part of it was lost. The strap fit, but the scene had no anchors and Lenna's established tools could not invent one.
- Thessha did not hand over repair hardware. She pointed toward possible hoist-crew stock and continued serving writs. The station did not reveal the needed fasteners on arrival, and only Dessek was present. He identified an existing maintenance bench and imposed his own political limit: help the sick rooms without stripping machinery that the Wardens could claim the occupying families dismantled.
- Human verdict is `CONTINUE_LANE`, not long-play acceptance. Scores are comprehension `5/5`, prose readability `4/5`, agency `5/5`, world aliveness `4/5`, and desire to continue `5/5`. Material causality and NPC boundaries are strong. Dessek's early exposition and multi-minute latency remain the main defects.
- The database contains one completed Opening and twenty completed player actions. All 67 model stages used `zai-coding-plan` `glm-5.2`, strict native objects, and attempt 1. Every stage was accepted. All 15 actor proposals were accepted. No retry, repair, fallback, provider switch, model switch, interruption, or Resume ran.
- SQLite integrity is `ok`, foreign-key check is empty, and runtime events are contiguous from `1` through `734` with no revision break. A full backend restart and actual browser reload preserved exact public-state hash `47286139ef5dbd59acee522427c8f378283fb515f1582c5550c74e8ed31e81d9`. The reloaded UI restored the same location, cast, possessions, narration, and choices. Browser warning and error capture is empty.
- GitNexus recovered through a clean full rebuild after the incremental FTS index failed. It is current at HEAD `8d33525` with `13813` nodes, `36834` edges, `790` clusters, and `300` execution flows. Query, context, process resources, and status work.
- r16 remains active at twenty of sixty eligible actions. Humanizer/deslop review kept technical proof, prose defects, latency, and the human play verdict separate.

Lane A r16, actions 21-30 and Judge suggested-target repair:

- Actions 21 through 28 form a complete material chain: Lenna identified exactly three masonry pins and a driver, received bounded permission, took and test-fit them, packed the existing repair stock, travelled back, installed one strap for a partial seal, then consumed the last two pins to stop the active draft. Residual sulfur smell remained. No tool, resource, or finished result appeared early.
- Actions 29 and 30 pivot from repair to social consequence. Thessha acknowledges fewer coughing households but grants no invented debt credit, continues serving writs, and names three households facing week-end collection. The next choices support knocking, seeking delay, visiting the shrine, or disengaging. Human verdict is `CONTINUE_LANE`: comprehension `5/5`, prose readability `4/5`, agency `5/5`, world aliveness `5/5`, and desire to continue `5/5`.
- The first action-30 attempt stopped before artifact acceptance because the one-target frozen-choice schema used tuple JSON shape. No world mutation occurred and Resume was not used. The interrupted test turn was backed up and removed atomically while the backend was offline; relevant immutability triggers were recreated verbatim and database/runtime invariants were rechecked.
- Judge now uses a standard exact-length array schema for frozen suggested targets and states that even one target must be returned inside a JSON array. Exact handle order remains compile-enforced. GitNexus impact is LOW and confined to Judge flows. Focused Judge tests pass `20/20`; backend typecheck passes. The same signed choice then completed through Play UI on GLM 5.2 attempt 1 and bound normally.
- The database contains one completed Opening and thirty completed player actions. All 100 stages are accepted attempt 1, all 19 actor proposals are accepted, commands match receipts, runtime sequences `1..1049` and revisions are contiguous, actor placements are valid, SQLite integrity is `ok`, and foreign-key check is empty. No retry, repair, fallback, provider or model switch, compatibility path, standalone smoke test, or Resume ran.
- Player causality remains visible across acquisition, travel, partial repair, completion, recognition, and the seizure decision. Independent Remane, Niruko, and Noro activity continued outside Lenna's perception; earlier independent Vassara movement became visible and led to the repair-material branch. Hidden events are not falsely credited with changing decisions that Lenna cannot yet observe.
- Actions 21 through 30 have median duration `122393 ms` and maximum `291462 ms`; latency remains a serious playability risk. Action 30 is specific and meaningful, with em-dash-heavy phrasing as a prose advisory. Restart and browser reload preserve public-state hash `65d93e85f0d7c66f89032ea25602aea606507ffea8ff3bd4b6e64751fe64035b`; browser warnings/errors and restart stderr are empty. Action-30 screenshot SHA-256 is `79ed6142b107d9bb2bb88a13519832a92d9bfa1dca952dd7cc947e7dc0a75b4e`.
- Humanizer/deslop review kept the Judge contract mechanical and separated stored proof, human prose judgment, and the latency defect. r16 remains active at thirty of sixty eligible actions.

Lane A r16, actions 31-40 and signed-petition state failure:

- Actions 31 through 33 found the only institutional route toward the seizure deadline. Thessha identified Valieri, the quota order, and the Compact Hall petition bench. The procedure remained understandable but repetitive, and two suggested actions kept returning to the already sealed flue.
- Action 34 moved Lenna to Winch-Shaft Station with a canonical `move_actor` receipt. Action 35 traced an upward passage only through `record_world_event`; prose reached the lamplit Hall tier while Rulebook placement remained at the station. The real graph is Winch-Shaft Station to Warden Tollhouse to Compact Hall. This is a strong spatial and narration `REVISE`.
- Actions 36 through 38 followed the real graph. Valieri was already at Warden Tollhouse investigating scored cable clamps while Ostane left for the station. This independent encounter is a world-aliveness `PASS`, though Valieri repeated too much of Thessha's procedure. Lenna then reached Compact Hall and found the clerk absent.
- Action 39 acquired one blank filing form through `adjust_actor_possession`. Action 40 wrote and signed the petition, but Game Master used only `advance_world_time` and `record_world_event`. The visible consequence and Narrator call it complete, dated, signed, unstamped, and unsubmitted while `Carrying` still says `blank filing form x1`.
- Disposition is `REJECT_R16_AT_ACTION_40`. r16 stops at forty signed and bound actions and will not resume or count toward a sixty-action claim. Scores are comprehension `5/5`, prose readability `3/5`, agency `5/5`, world aliveness `5/5`, and desire to continue `4/5`. The terminal failure is the durable possession contradiction, not the prose score.
- Actions 31 through 40 took `2078429 ms`, with median `214740 ms` and maximum `283323 ms`. All 35 stages used `zai-coding-plan` `glm-5.2`, strict native objects, and attempt 1. Eleven actor proposals were accepted. Commands and receipts match `34/34`; no retry, repair, fallback, provider switch, model switch, interruption, or Resume ran.
- The campaign has one completed Opening, forty completed player actions, 135 accepted attempt-1 stages, and 30 accepted actor proposals. World version is `71`, runtime revision is `1450`, runtime events are contiguous from `1` through `1450`, SQLite integrity is `ok`, and foreign-key check is empty.
- Backend restart and browser reload preserved public-state hash `56ce756dae00a22b386408c6fecc2f08cb0122fd1cc25ab33243232a3b0b12ea` and the same completed-petition versus blank-form contradiction. Restart stderr is empty. Humanizer/deslop review kept the route advisory, taste findings, and terminal state failure distinct.

Lane A r17, possession repair and action 1:

- The accepted action-40 artifacts proved that Judge targeted the blank form correctly, while Game Master's contract had no retained-object transformation. The new model-authored `transform` operation compiles to an atomic protected source spend and directly perceived result acquire. No prose parser, semantic regex, retry, repair, fallback, compatibility path, or backend-authored item name was added.
- Focused Game Master tests pass `26/26`, turn-runtime tests pass `36/36`, and backend typecheck passes. GitNexus impact is `LOW` for the proposal/compiler/prompt and `MEDIUM` for the exported factory, with six direct consumers and no signature change.
- r17 materialized the same accepted Lowwater Ledger template with zero characters and zero turns. Lenna was created through the real Character UI with a blank form, ink pot and stylus, and repair kit. One Opening and one signed player action completed through the normal UI on `zai-coding-plan` `glm-5.2`, strict objects, and attempt 1 only.
- Action 1 transformed the blank form into `Work ledger, Day 1 - Debt-Ward Tenements`. Three applied receipts advanced time, reduced the blank form to zero, and acquired the ledger at one. The ink pot with stylus and repair kit remained at one. The rendered `Carrying` panel matches Rulebook truth.
- Opening prose offered craft work from Vassara's own shortage and kept a separate medical problem visible. Action prose preserved the useful ledger entry; one phrase is stiff but clear. Scores are comprehension `5/5`, prose readability `4/5`, agency `5/5`, world aliveness `4/5`, and desire to continue `5/5`.
- Restart and browser reload preserved public-state hash `e9bef56790a05f865f59ec367a1acda74c66f2d1be8ac09088d4872a2024161d`; restart stderr is empty. Runtime sequences are contiguous `1..92`, SQLite integrity is `ok`, and foreign-key check is empty. Screenshot SHA-256 is `1657e363742dae82e1fe2904b7da8ba7562047ff1eac05bdf45cbcfb2e009eae`.
- r17 is active at one of sixty eligible actions. Humanizer/deslop review keeps the targeted product proof distinct from the still-unmet long-lane acceptance.

Lane A r17 actions 2-7 and roster-bound lead diagnosis:

- Vassara created a concrete Dael-household lamp job, named Anni, and quoted possible saltfish payment, but no Anni or Dael actor exists in the accepted roster. Two consent-first contacts therefore produced only the silence required by the ambient-contact authority boundary.
- Action 6 returned to Vassara. She correctly denied authority over another household's lamp, but still repeated Anni and proposed waiting for unnamed residents. Action 7 forced an honest answer: no available person could be named and no sure work existed. The result still repeated the prior prose-only name and suggested another ambient doorway.
- Game Master now receives the accepted person roster as `CANONICAL_PEOPLE`. The prompt treats it as identity validation only, rejects prior prose as actor authority, forbids an unlisted resident from owning a transaction or future reply, and forbids knocking, calling, or waiting for one as the offered next step. No parser, retry, repair, fallback, provider switch, compatibility path, or backend-authored prose was added.
- Focused Game Master tests pass `26/26`, backend typecheck passes, SQLite integrity is `ok`, and foreign-key check is empty. Actions 6 and 7 used seven accepted attempt-1 GLM 5.2 stages, including one Actor Replanner. Screenshot SHA-256 values are `cf7a599e14ffb74d3fc72b662675ba696945e387b0e10043535b3ffc18396ac4` and `4d82d70fb2ad80f462e8ead7008babe1d44f490b4d2a467db38e3e0c0345a445`.
- Human verdict is `FREEZE_R17_AS_DIAGNOSTIC_AFTER_ACTION_7`: comprehension `5/5`, prose readability `4/5`, agency `3/5`, world aliveness `2/5`, desire to continue `3/5`. The branch is understandable but not playable enough for pristine acceptance. A fresh template copy must test the first person-specific offer under the repaired prompt.

Lane A r18 clean roster-bound offer proof:

- r18 reused `lowwater-ledger-pristine-fb2c7074` with zero characters and turns; world generation did not run. Lenna was rebuilt through the rendered Character UI with her three starting possessions, then entered through explicit `Lower Wards / Outsider / Just arrived / Following a lead` conditions.
- Opening Vassara owned the offer herself: salves and a dry corner for repaired corridor panels. The signed first action forced exact authority, success, payment, and acceptance terms. Vassara selected the split housing over her own door, defined a working seal through draft, damp, and token behavior, offered one salve packet per panel or one dry night per two, and retained acceptance authority.
- This is a clean product `PASS` for the roster rule. No prose-only person owns the job or future reply. Scores are comprehension `5/5`, prose readability `4/5`, agency `5/5`, world aliveness `4/5`, and desire to continue `5/5`.
- All five opening and action stages used `zai-coding-plan` `glm-5.2`, strict native objects, and attempt 1. SQLite integrity is `ok`, foreign-key check is empty, and runtime events and revisions are contiguous `1..99`. Restart and browser reload preserved public hash `c0a3990df3aa412d02257a396027e530e8ab7267ad8a1219988213fc4da193a0`; restart stderr is empty.
- Opening and action-1 screenshot hashes are `c953c5a5fda70749ee1eb1686b53c93aecfbdb827131cdd46392f11695c938fc` and `7d54117fc5a2d3c3b731279e6b30d448d5ce4964128588506b50761415959297`. r18 is now the active replacement lane at one of sixty eligible actions.

- [ ] Unchecked R7 repair/qualification note (2026-08-19): Game Master's provider transport now uses strict effect-kind partitions plus an exact `effectOrder`; private decoding rejects duplicate, missing, out-of-range, wrong-kind, uncovered, echoed, and irrelevant fields before the unchanged domain schema. Focused Game Master verification passes `84/84`, including native/tool parity for duplicate and unmatched resource rows; backend typecheck, backend build, frontend build, and `git diff --check` pass. GitNexus impact at accepted base `fc551f5` is MEDIUM (17 impacted files, 8 direct importers, no indexed processes); the exported error contract was excluded and unchanged. Humanizer/deslop review kept the provider wording literal and mechanical with no player-facing copy change.
- Direct provider qualification accepted the required possession acquisition and obligation kind `none` on `zai-coding-plan/glm-5.3`, tool mode, attempt 1, with no fallback, retry, or provider/model substitution. A fresh supported-provenance clone then reached a model-authored Opening, accepted one signed player action once, completed one accepted Game Master plan and one `model_accepted` narration operation/proper scene, and reloaded the built Play page with the same rendered scene and coherent API/SQLite state. SQLite reports `integrity_check=ok` and an empty `foreign_key_check`; final API projection hash is `413b76586b8f8ae01b342096eabf248b06fbf4ec2bbee580f5a1ad1663648401`.
- The adjacent application/turn-runtime check executed 118 assertions but exited on the known Vitest worker unhandled `Timeout calling "onTaskUpdate"`; no relevant assertion failed. Character generation was not used for the journey after a separate provider `powerStats` contract defect; the valid player draft was installed once through the normal player API before the built UI Opening. R7/R7k endurance and replay remain intentionally unstarted.
- [ ] Unchecked R7 repair/qualification note (2026-08-19): Stage 4 PowerStats now uses one strict six-field `powerStatsGenerationSchema` contract for imported and original characters, with imported attempts capped at one and original automatic same-stage attempts capped at three; loose normalization, text fallback, and repair are absent from the original path. Focused ingestion/schema coverage passes `30/30`, the directly affected character suites pass `116/116`, backend typecheck/build and frontend production build pass, and `git diff --check` is clean.
- A fresh supported provenance clone `qualification-freeform-powerstats-20260819-d` prepared once before character creation. The exact original Mara Quill concept was entered once and Generate was submitted once; Stage 3 and Stage 4 both requested/used `zai-coding-plan/glm-5.3` in strict tool mode with no fallback, repair, or substitution. The rich draft rendered, Continue was used once, exactly one generated player character persisted, the flow reached `opening_required`, and one same-page reload preserved that state. Query-only SQLite checks returned `integrity_check=ok` with no foreign-key violations; transient `powerStats` remained omitted from the persisted character record. R7 endurance was not launched.
- [ ] Unchecked R7 note (2026-08-19): Campaign Play runner/evidence now has crash-resumable signed-decision binding and explicit immutable, path-qualified provenance packages; only a bounded qualification was attempted. R7 remains unchecked. No player-facing prompt or copy change is expected.
