...
function renderMarkdown(input: ReturnType<typeof validatePhase94ReportArtifacts>): string {
  ...
  const lines = [
    ...
    "",
    "## Observability Evidence Policy",
    "",
    `- version: ${report.observabilityEvidencePolicy.version}`,
    `- owner: ${report.observabilityEvidencePolicy.owner}`,
    `- gameplay authority: ${report.observabilityEvidencePolicy.gameplayAuthority}`,
    `- raw local-only kinds: ${report.observabilityEvidencePolicy.rawLocalOnlyKinds.join(", ")}`,
    `- remote public kinds: ${report.observabilityEvidencePolicy.remotePublicKinds.join(", ")}`,
    "",
    "## Routes",
    "",
