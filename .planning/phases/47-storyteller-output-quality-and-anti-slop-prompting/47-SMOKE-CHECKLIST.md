# Phase 47 Live GLM Smoke Checklist

Use the configured GLM storyteller path and judge the visible narration only. The goal is not "literary"; it is playable RP prose with cleaner scene motion and fewer obvious model tells.

## Approval Rule

Mark the phase `approved` only if all four scene types pass and none of the failure modes below show up in a way that breaks immersion.

## Failure Modes This Phase Guards

- repeated lead: the same opening beat or first sentence restarts in a later paragraph
- header or tool leak: bracketed sections, prompt scaffolding, or tool-call syntax reaches the player-facing prose
- instruction echo: narration starts quoting or paraphrasing narrator instructions instead of narrating the scene
- slop cluster: obvious announcement opener plus formulaic contrast or rhetorical setup in the same sample
- omniscience: the narrator states identity, intent, or hidden knowledge the player could not perceive

## Scene Checks

### 1. Combat / action-heavy turn

- Does the text move through concrete action and consequence instead of pausing to editorialize?
- Does it avoid restarting the same beat after impact lands?
- Does it stay readable under pressure instead of inflating into purple prose?

### 2. Dialogue / negotiation turn

- Does the prose let the conversation carry the scene instead of narrating around it?
- Do emotional beats land once instead of being restated?
- Does the narrator avoid assistant-like meta phrasing or instruction echo?

### 3. Quiet / scene-setting turn

- Does it establish the place with specific details instead of generic atmosphere filler?
- Does it stay restrained instead of writing like a dramatic monologue?
- Does it avoid "Here's the thing", "The truth is", and similar announcement openers?

### 4. Eerie / tension-heavy turn

- Does it create tension through signals, pacing, and consequence rather than melodramatic inflation?
- Does it preserve knowledge limits and hidden presence boundaries?
- Does it avoid formulaic "it's not X. It's Y." style contrast writing?

## Pass Criteria

- no repeated lead paragraph
- no prompt/header/tool leakage
- no instruction echo
- no player-action impersonation
- no omniscient identity or intent leakage
- noticeably less purple prose and generic assistant smell than pre-47 output

## Result

- [ ] combat approved
- [ ] dialogue approved
- [ ] quiet approved
- [ ] eerie approved
- [ ] phase 47 approved
