...
Status: complete
Owner: A1
Scope: lock A6 to `architecture.md` and completed A4/A5a/A5b outputs.
Output:
...
Status: complete
Owner: A2
Scope: inspect existing campaign kernel types and adapters.
Output:

- [inspected] `CampaignWorldGraph`, `CampaignWorldNode`, and `CampaignWorldEdge` already support `Character` and `located_at`.
- [inspected] `CampaignCastMember.id` can serve as the graph character node id.
- [inspected] `CampaignCastPlacement` carries `locationId`, `sceneLocationId`, and notes.
...
- [proposed] Character node id equals `CampaignCastMember.id`.
...
- Call `buildCampaignWorldGraph` with the existing kernel graph and cast registry.
- Persist the returned graph.
- Keep the phase unchanged.
- Expose the connector through `POST /api/kernel/campaigns/:id/graph/compose`.
