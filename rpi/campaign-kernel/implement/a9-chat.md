...
- Produce `CampaignGmResponse` with text, suggested actions, and soft state hints.
...
- Expose `POST /api/kernel/campaigns/:id/chat/message`.
...
- [executed] Focused campaign kernel backend suite passed: 13 files, 69 tests.
...
- Current A9 code covers the review items: `createCampaignChatMessage` reads `runtimeState.currentSceneId`, persists `chatSession.pendingSoftStateHints`, uses the revised freeform line, and formats Look output as scene, present cast, and routes on separated lines.
