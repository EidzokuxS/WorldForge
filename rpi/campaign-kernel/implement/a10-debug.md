...
Expose a backend debug snapshot for the campaign kernel kernel so playtests can inspect the current scene, cast, routes, chat turns, and pending soft state hints.
...
- Expose `GET /api/kernel/campaigns/:id/debug`.
...
- [executed] Focused campaign kernel backend suite passed: 14 files, 73 tests.
