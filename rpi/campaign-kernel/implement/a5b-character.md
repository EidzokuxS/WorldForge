# A5b campaign kernel character wiring canvas

Objective:
- Wire player character create/import into the Campaign Kernel without calling the old `/api/worldgen/*` character routes from the campaign forge UI.

Operating contract:
- Campaign Kernel owns a new API surface under `/api/kernel`.
...
- `frontend/app/(non-game)/campaign/[id]/forge/__tests__/page.test.tsx`

Non-goals:
- Do not save the player into the old `players` table.
- Do not route the campaign forge page through `/campaign/:id/character`.
- Do not call frontend helpers `saveCharacter`, `parseCharacter`, `generateCharacter`, `researchCharacter`, or `importV2Card` from the campaign forge page.
...
Output:
- [inspected] `architecture.md` names A5b as `Campaign Kernel import/create character wiring`.
- [inspected] A5a already provides `buildCampaignCastRegistry`.
...
- [decided] Campaign Kernel uses new frontend helpers and new `/api/kernel/*` routes.
...
- [inferred] Character ingestion can remain a shared service if the campaign kernel route owns the API boundary and persistence.
...
Output:
- [proposed] `saveCampaignPlayerCharacter(campaignId, draft, source)` reads an existing kernel or creates a draft kernel from campaign config.
...
- [proposed] Frontend test: campaign forge page renders player creation controls and calls campaign kernel helpers, with old character helpers absent from mocks.
- [proposed] Static search: campaign forge page must not import old frontend character helpers.
...
- [rejected] Reusing the old character page as the campaign forge UI because it saves old runtime state.
