...
- Removed ignored old non-campaign kernel RPI folders and generated `frontend/next-env.d.ts`.
...
- Largest local directories after purge:
  ...
  - `backend/`: 10.00 MB
  - `frontend/`: 1.61 MB
  - `rpi/`: active `campaign kernel` scope only
...
- `git diff --cached --name-status`: only `.env.example`, `.gitignore`, `AGENTS.md`, `rpi/campaign-kernel/research/*`, `tasks/lessons.md`, and `tasks/todo.md`.
