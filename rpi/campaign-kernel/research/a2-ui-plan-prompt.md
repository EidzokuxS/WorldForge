...
Review the WorldForge campaign kernel A2 UI/UX plan. Return a concise implementation verdict and any required UI corrections.

# Architecture Direction

The campaign kernel path starts with a clean Campaign Kernel shell:
...
- The wizard calls `POST /api/campaigns`, loads the created campaign, clears the draft session, and routes to `/campaign/:id/forge`.
- The campaign forge page shows campaign name, premise, kernel phase, turn index, empty graph counts, and the boundary text that full worldgen is parked outside the route.
