...
Clean the repository state on `feat/revamp` so the upcoming mechanics revamp is easy to inspect, review, and navigate.
...
Status: complete
Owner: A3
Scope: Identify stale planning, RPI, task, docs, and proof artifacts that distract from revamp.
Output:
