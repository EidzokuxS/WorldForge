...
Build a pure adapter from current `ScaffoldLocation[]` data into revamp `RevampWorldGraph` location nodes and spatial edges.
...
Status: complete
Owner: L1
Scope: Bind A4 to the revamp architecture.
Output:

- [sourced] Development order says A4 is `Locations adapter from current worldgen`.
- [sourced] Revamp graph supports `Location`, `SceneLocation`, `route_to`, and `located_at`.
...
- [proposed] `description`, normalized `tags`, `isStarting`, `kind`, and `parentLocationName` land in `RevampWorldNode.data`.
