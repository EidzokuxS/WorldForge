...
Build A5a: a pure adapter that turns existing `CharacterDraft` and `ScaffoldNpc` inputs into the revamp `RevampCastRegistry`.
...
- [sourced] Revamp architecture says Cast Registry owns one player, imported cast, and generated cast.
- [proposed] A5a delivers the registry adapter; A5b wires import/create flows into the revamp path.
...
- +1 decision: A5a is a pure registry adapter. A5b will wire import/create flows into the revamp path.
