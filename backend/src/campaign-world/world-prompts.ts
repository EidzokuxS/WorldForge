...
const worldConnectionsOutputGuidance = `Keep each pressure name <=64 characters, description to one sentence <=160 characters, and trajectory to one sentence <=120 characters. Create 3 or 4 distinct pressures grounded in the source, using only the concrete actor and location anchors each pressure needs.`;
...
export function buildWorldConnectionsTransportPrompt(
  ...
  const relationSlots = Array.from({ length: actorCount }, (_, index) => index);
...
Return exactly the top-level keys relations and pressures, with no extra keys. Each relations[] object contains exactly relationSlotIndex, targetActorIndex, relationType, and intensity. "relationType" must be exactly one of alliance, rivalry, authority, dependency, kinship, association, or hostility. "relationSlotIndex" is the fixed source slot: return exactly one relation row for every integer in RELATION_SLOTS, exactly once. Do not skip or repeat a slot. "targetActorIndex" must be copied as one direct integer from ALLOWED_ACTOR_INDICES and must differ from relationSlotIndex. Never calculate or derive the target from the source index. Every actor therefore participates as a source exactly once, and every target is an explicit actor index. Use the complete WORLD_FRAME descriptions, location tags, route endpoints and travelCost values, and each actor's exact name, role, summary, objective, presentLocationIndex, and homeLocationIndex to choose meaningful relation endpoints and pressure anchors. Code builds each persisted relation summary from those accepted actor names and relationType; do not return summary, rationale, actor names, or any other relation prose. Each pressures[] object contains exactly name, description, trajectory, urgency, actorIndices, and locationIndices. Use only integer actor indices from WORLD_CAST_SKELETON and persistent location indices from PERSISTENT_LOCATION_SLOTS; actor indices are integers from 0 through ${Math.max(0, skeleton.actors.length - 1)}, and persistent location indices are integers from 0 through ${Math.max(0, locationSlots.length - 1)}. Never emit actorRef, locationRef, or any free-form reference. Each pressure must anchor at least one actor index and one persistent location index. At least one pressure must contain both a location index from STARTING_MACRO_SCENE_INDICES and a support actor whose presentLocationIndex is included in that same pressure's locationIndices. A pressure that names the starting scene without that support actor, or names a support actor anchored to another scene, fails the contract. Code restores stable references and runs the unchanged strict connections validator.
