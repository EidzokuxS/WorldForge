...
Return exactly one object in each slot. Every actor object contains exactly these six keys in this order: name, role, summary, presentLocationIndex, homeLocationIndex, objective. keyActorOne and keyActorTwo use role key; startingSupport and supportActor use role support; remoteBackground and backgroundActor use role background; otherActorOne and otherActorTwo use role key, support, or background. Use no omitted or extra actor keys. Set startingSupport.presentLocationIndex to the first persistent scene under the starting macro and remoteBackground.presentLocationIndex to the first persistent scene outside it. Every other presentLocationIndex must be an integer from 0 through 5. Every homeLocationIndex must be -1 or an integer from 0 through 5; use -1 for no home scene. Code applies the existing skeleton minima, spread, anchor-placement, and player-identity reservation checks after it decodes the frame.
...
Return exactly these eight top-level actor slots, in this order: keyActorOne, keyActorTwo, startingSupport, supportActor, remoteBackground, backgroundActor, otherActorOne, otherActorTwo, with no extra keys. Every slot is one object containing exactly these six keys in this order: name, role, summary, presentLocationIndex, homeLocationIndex, objective. keyActorOne and keyActorTwo use role key; startingSupport and supportActor use role support; remoteBackground and backgroundActor use role background; otherActorOne and otherActorTwo use role key, support, or background. Use no omitted or extra keys. Copy startingSupport.presentLocationIndex exactly from STARTING_SUPPORT_SLOT.index and remoteBackground.presentLocationIndex exactly from REMOTE_BACKGROUND_SLOT.index. Every other presentLocationIndex is an integer from 0 through ${Math.max(0, locationSlots.length - 1)}. Use homeLocationIndex -1 when no home scene is needed; otherwise use an integer from 0 through ${Math.max(0, locationSlots.length - 1)}. Compare all names after case/whitespace normalization and keep them unique; names must identify distinct people rather than repeated aliases.

Ground each anchor's name, summary, and objective in its assigned scene's name and concrete world context. Never mention slot indices, array positions, parentMacroIndex, macro numbers, schema fields, or transport mechanics in any name, summary, or objective. All names, summaries, and objectives remain model-authored canonical fields and are preserved unchanged into relations, pressures, and the final cast. Use integer indices from the supplied location slots only in presentLocationIndex and homeLocationIndex; never emit actorRef, locationRef, or any free-form reference. Keep presentLocationIndex values varied for non-anchor rows when the frame offers multiple scenes. Keep every summary concrete enough to ground a social pressure and every objective concrete enough to drive a playable choice; do not create a group as an actor. Do not return traits, tags, motivation, horizon, priority, additional goals, or any other detail-stage field.`;
}
