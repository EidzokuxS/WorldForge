...
function allCastMembers(kernel: CampaignKernel): CampaignCastMember[] {
  return [
...
function castById(kernel: CampaignKernel): Map<string, CampaignCastMember> {
...
}): CampaignDebugSceneSummary | null {
...
function presentCast(input: {
  kernel: CampaignKernel;
  members: ReadonlyMap<string, CampaignCastMember>;
}): CampaignDebugCastSummary[] {
  const setup = input.kernel.startingSetup;
  if (!setup) {
    return [];
...
function routes(input: {
  kernel: CampaignKernel;
  nodes: ReadonlyMap<string, CampaignWorldNode>;
  scene: CampaignDebugSceneSummary | null;
}): CampaignDebugRouteSummary[] {
  if (!input.scene) {
    return [];
  }
...
export function buildCampaignDebugSnapshot(
  kernel: CampaignKernel,
  recentTurnLimit = 8,
): CampaignDebugSnapshot {
