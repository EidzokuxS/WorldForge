...
function makeImportedNpc(): CampaignCastMember {
  return {
...
describe("campaign kernel cast kernel", () => {
  beforeEach(() => {
  ...
  it("reads an existing kernel or creates a draft view from campaign config", () => {
    ...
    const kernel = readOrCreateCampaignKernel(CAMPAIGN_ID);
    ...
    expect(fs.existsSync(getCampaignKernelPath(CAMPAIGN_ID))).toBe(false);
  });

  it("saves a player CharacterDraft into cast registry and advances to cast_ready", () => {
    writeConfig();

    const kernel = saveCampaignPlayerCharacter({
      campaignId: CAMPAIGN_ID,
