...
function makeRegistry(): CampaignCastRegistry {
  return {
...
function makeGraph(): CampaignWorldGraph {
  return {
    nodes: [
      {
        id: "scene:platform",
...
function makeSetup(): CampaignStartingSetup {
  return {
    mode: "gm_invented",
...
function build(message: string) {
  return buildCampaignGmResponse({
...
describe("campaign kernel chat gm", () => {
  ...
  it("does not import old chat, generator, provider, or player-table implementations", () => {
    const source = fs.readFileSync(
      path.resolve(process.cwd(), "src/campaign-kernel/chat-gm.ts"),
      "utf-8",
    );
