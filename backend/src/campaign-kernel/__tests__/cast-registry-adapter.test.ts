...
import { buildCampaignCastRegistry } from "../cast-registry-adapter.js";
...
describe("campaign kernel cast registry adapter", () => {
  it("builds player, imported, and generated cast buckets", () => {
    ...
    const registry = buildCampaignCastRegistry({
      playerCharacter: {
        draft: player,
  ...
  it("does not import old route or generator implementations", () => {
    const source = fs.readFileSync(
      path.resolve(process.cwd(), "src/campaign-kernel/cast-registry-adapter.ts"),
      "utf-8",
    );
