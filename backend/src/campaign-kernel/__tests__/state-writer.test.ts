...
import { applyCampaignStateHints, applyCampaignStateWriter } from "../state-writer.js";
...
describe("campaign kernel state writer", () => {
  beforeEach(() => {
  ...
  it("applies a route intent as MoveCharacter", () => {
    const result = applyCampaignStateHints(makeActiveKernel());
  ...
  it("rejects unsupported hints and clears the pending list", () => {
    ...
    const result = applyCampaignStateHints(kernel);
  ...
  it("fails when route intent has no visible route", () => {
    ...
    expect(() => applyCampaignStateHints(kernel)).toThrow(
  ...
  it("persists state writer output", () => {
    ...
    const result = applyCampaignStateWriter({ campaignId: CAMPAIGN_ID });
