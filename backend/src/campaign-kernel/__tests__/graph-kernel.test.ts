...
import { composeCampaignKernelWorldGraph } from "../graph-kernel.js";
...
function makeMember(name: string, role: "player" | "npc" = "player"): CampaignCastMember {
  return {
...
describe("campaign kernel graph kernel", () => {
  beforeEach(() => {
  ...
  it("composes cast into the kernel world graph without changing phase", () => {
    ...
    const kernel = composeCampaignKernelWorldGraph(CAMPAIGN_ID);
  ...
  it("fails before cast_ready", () => {
    ...
    expect(() => composeCampaignKernelWorldGraph(CAMPAIGN_ID)).toThrow(
