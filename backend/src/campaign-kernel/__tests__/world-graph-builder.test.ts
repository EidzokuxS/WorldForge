...
function makeMember(
  name: string,
  overrides: Partial<CampaignCastMember> = {},
): CampaignCastMember {
  const role = overrides.campaignRole === "player" ? "player" : "npc";
  const member: CampaignCastMember = {
...
function makeBaseGraph(): CampaignWorldGraph {
  return {
    nodes: [
      {
...
describe("campaign kernel world graph builder", () => {
  it("adds character nodes and placement edges while preserving the base graph", () => {
    ...
    const graph = buildCampaignWorldGraph({
      baseGraph,
      castRegistry: {
  ...
  it("does not import old route or generator implementations", () => {
    const source = fs.readFileSync(
      path.resolve(process.cwd(), "src/campaign-kernel/world-graph-builder.ts"),
      "utf-8",
    );
