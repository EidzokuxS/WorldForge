...
import { buildCampaignDebugSnapshot } from "../debug-snapshot.js";
...
describe("campaign kernel debug snapshot", () => {
  it("summarizes a draft kernel without setup", () => {
    const snapshot = buildCampaignDebugSnapshot(createDraftCampaignKernel({
      id: "campaign-a10",
      premise: "A railway city under curfew.",
    }));
  ...
  it("summarizes active scene, present cast, routes, turns, and pending hints", () => {
    const snapshot = buildCampaignDebugSnapshot(makeActiveKernel(), 2);
  ...
  it("fails when setup references missing present cast", () => {
    ...
    expect(() => buildCampaignDebugSnapshot(kernel)).toThrow(
