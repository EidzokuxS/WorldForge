...
type ChatIntent =
  | { type: "look" }
  | { type: "talk"; member: CampaignCastMember }
  | { type: "route"; target: CampaignWorldNode }
  | { type: "freeform" };
...
function visibleRoutes(input: {
  ...
  return input.graph.edges
    .filter((edge): edge is CampaignWorldEdge => (
...
function suggestedActions(input: {
  present: CampaignCastMember[];
  routes: CampaignWorldNode[];
}): string[] {
...
function describePresent(present: CampaignCastMember[]): string | null {
  if (!present.length) {
    return null;
  }
...
function describeRoutes(routes: CampaignWorldNode[]): string | null {
  if (!routes.length) {
    return null;
  }
...
function responseForIntent(input: {
  intent: ChatIntent;
  scene: CampaignWorldNode;
  sceneDescription: string;
  present: CampaignCastMember[];
  routes: CampaignWorldNode[];
  userMessage: string;
}): { text: string; hints: CampaignSoftStateHint[] } {
...
export function buildCampaignGmResponse(input: BuildCampaignGmResponseInput): CampaignGmResponse {
