...
function buildGmReadSystemPrompt(): string {
  return [
    ...
    "Use tool_plan when the turn needs an accepted backend action checklist for route checks, movement, visible dialogue outcomes, concrete entity tag changes, or a scene-local beat receipt.",
    ...
    "For this live slice, checklistRequest.requiredEffectKinds may use only route_check, movement, dialogue_outcome, entity_tag, or scene_beat.",
    ...
    "If the player action needs unimplemented mutation, combat, hidden knowledge, durable dialogue memory, or a backend capability outside route_check/movement/dialogue_outcome/entity_tag/scene_beat, choose clarification.",
  ].join("\n");
}
...
function buildToolRequestSystemPrompt(): string {
  return [
    ...
    "Use only clean v2 tool ids: route.check.v2, actor.move.v2, dialogue.record.v2, entity.tag.v2, or scene_beat.record.v2.",
