...
export function buildGmReadPromptContract(options: {
  ...
  return [
    ...
    "Fuzzy bridge policy: bridge understandable low-risk navigation, search, and service-role intent with listed legal candidates and bridge lookup/state tools; do not clarify solely because the player's phrasing is not an exact backend string.",
    "For fuzzy movement/POI/search/service requests, prefer legal low-risk advancement: use current candidates or lookup observations, then choose tool_plan when movement, search, minor POI, or support-extra state must be grounded.",
    "If several legal candidates are materially similar, choose the best grounded candidate and proceed. If risk, cost, or target identity materially differs, ask a bounded diegetic 2-3 choice question.",
    "Clarification is allowed only for materially different risk/cost, irreversible high-impact action, contradictory intent, mechanically important target identity, or no fair playable bridge.",
    "Do not ask exact ID, backend target, route id, or connected-location questions when listed candidates or allowed bridge tools can fairly resolve the turn.",
