...
const runtimeToolInputShapes = {
  ...
  record_dialogue_outcome: [
    ...
    "Every dialogue outcome requires quote or at least one structured claim; summary is support/display only and is never final-narration authority.",
    "Durable outcomes require futureUseKind and futureRelevance. Durable procedural answered/warned/redirected outcomes require quote and at least one claim.",
    "With a visible speaker/speakerRef, never use unavailable/no_current_answer; if that speaker cannot answer, use refused, redirected, silent, gestured, warned, or answered with quote or claims.",
    "Use unavailable/no_current_answer only when no visible/current speaker or source exists: omit speakerRef, use authorityKind no_visible_authority, include requestedRoleText, and include quote or claims.",
  ],
  record_world_fact: [
...
export function buildGmReadPromptContract(options: {
  ...
  return [
    ...
    "Use direct only for greetings, banter, local description, sensory color, or ambient reaction that creates no playable NPC claim. If a visible/current NPC answers, refuses, redirects, warns, bargains, or meaningfully responds to a player question/request, choose tool_plan with dialogue_outcome even when requiresStructuralEffect=false.",
    ...
    "Direct, continue, and clarification are checked by a separate no-mutation admissibility gate. Do not label route, routes/doors/stairs, proof, document, permission, procedure, public-service status, movement, possession, actor creation, combat/threat, or durable world/social claims as ordinary local response; the harness will repair them into grounded paths.",
    "Use tool_plan when world state must actually change, a fact must matter later, a scene affordance must be established, an NPC/source response must be recorded, reusable NPC procedural/logistical information must be recorded, or a support actor/location must exist for the current fiction.",
    ...
    "If the player asks a visible authority, guard, clerk, warden, worker, assistant, witness, service role, or any visible/current NPC for proof requirements, proofs, permits, waivers, authorisations, permissions, rules, procedure, route status, dispatch rules, what changed, what changed today, why something was given, what an item/contact means, which posted item/notice/rule/sign applies to a document or case, or any answer the player can rely on later, choose tool_plan. Do not use direct for reusable or contentful NPC answers.",
    ...
    "If the player first follows/goes/walks/descends to a distinct named place and then asks, shops, or speaks there, choose tool_plan with runtimeRequirement { \"kind\": \"state_mutation\", \"effectKind\": \"movement\" } for the travel beat. Do not use dialogue_outcome plus create_scene_extra to pretend the destination responder is in the current scene.",
    "If the player explicitly goes, walks, exits, enters, returns, or travels to a named/listed destination, choose tool_plan with runtimeRequirement { \"kind\": \"state_mutation\", \"effectKind\": \"movement\" } even when the route is risky, watched, or contested. Oracle may decide risk, notice, or resistance, but Oracle alone cannot change current location or prove arrival.",
    ...
    "Use roll_oracle for uncertain visible reactions or resistance when no new durable fact or location change is needed yet: belief, witness notice, alarm, or visible physical traction. Do not use roll_oracle as the sole path for completed movement or arrival.",
