...
export function deterministicSettlementBridge(view: CleanNarratorView): string {
  ...
  if (movementFact) {
    const location = movementFact.text
      ...
      .replace(/\.$/u, "");
    return `Вы перемещаетесь в ${location}.`;
  }
  ...
  if (route) {
    return route.backendFacts[0]?.text ?? route.text;
  }

  const failed = view.stepAuditForGrounding[0];
  if (failed) {
    return `Это действие сейчас нельзя подтвердить: ${failed.publicReason}`;
  }

  const firstFact = view.acceptedEvidence[0]?.backendFacts[0]?.text;
  return firstFact ?? "Действие зафиксировано без новых подтверждённых изменений.";
}
