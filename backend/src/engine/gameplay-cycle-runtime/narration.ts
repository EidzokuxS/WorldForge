...
function preferredPromptFacts(evidence: AcceptedNarrationEvidence): AcceptedNarrationBackendFact[] {
  ...
  if (evidence.claimKinds.includes("device_surface_observation")) {
    if (evidence.claimKinds.includes("device_surface_unavailable")) {
      return preferredPromptFactsByRole(evidence, [
        "device_surface_beat",
        "device_label",
        "anchor_scene",
...
function selectRouteChoiceFactRefs(move: CleanNarratorPageTaskMove): string[] {
  const routeOriginFactRefs = sentencePlanFactRefsByRole(move, ["route_origin"]);
  if (routeOriginFactRefs.length > 0) return routeOriginFactRefs;
...
function selectPlayerLocalConditionFactRefs(move: CleanNarratorPageTaskMove): string[] {
  return sentencePlanFactRefsByRole(move, [
    "player_condition_operation",
    "condition_key",
    "current_scene_anchor",
...
function selectTurnEventFactRefs(move: CleanNarratorPageTaskMove): string[] {
  ...
  if (move.entryProseCues.includes("player_local_condition")) {
    return selectPlayerLocalConditionFactRefs(move);
  }
  ...
  if (move.entryProseCues.includes("device_surface_observation")) {
    return sentencePlanFactRefsByRole(move, [
      "device_surface_beat",
      "device_label",
      "anchor_scene",
...
function sentencePlanAdventureDetailPalette(
  ...
  if (beatObjective === "render_route_choices") {
    return ["route_origin", "structured_route_handles"];
  }
...
function sentencePlanAdventureCue(
  ...
  return {
    ...
    detailPalette: sentencePlanAdventureDetailPalette(proseMaterials, beatObjective),
  };
}
  ...
  switch (move.proseMove) {
    ...
    case "render_authoritative_turn_event":
      if (
        ...
        move.entryProseCues.includes("player_local_condition")
        && (move.entryProseCues.includes("local_observation") || move.entryProseCues.includes("bounded_visibility_negative"))
      ) {
        pushPlan(
          "turn_event_beat",
          move.coverage,
          selectPlayerLocalConditionFactRefs(move),
          "render_player_condition",
        );
        pushPlan(
          "turn_event_beat",
          move.coverage,
          selectLocalObservationFactRefs(move),
...
export function buildCleanNarrationSystemPrompt(
  ...
  return [
    ...
    "Opening scene handling: opening local_lens facts prove present placement only, and opening action_handle facts prove the handoff question only. Render them as a played scene: immediate floor-level place pressure, one visible/sensory surface beat when softProseBudget allows it, then the player-facing question or action handoff. Treat opening fact summaries as raw material, not finished copy; the first page must not sound like a receipt, route list, map legend, or backend report.",
    ...
    "Choice presentation: promptInput.narrativePageTask.choicePresentation carries structured route choices for action handles. Treat choices as UI/action-handle data, not prose copy. Ordinary route-choice prose should summarize the visible opportunity to move from the accepted origin without enumerating destination labels. Preserve a route label verbatim only when the current player action or a route_status sentence is about that one named route. costHandling names exact travel-cost material available for tactical timing; omit costs from ordinary route-handoff prose unless the sentence explicitly states timing and cites the cost facts. Anchor through route origin as a place label when anchorStyle=route_origin_place_label; reserve posture verbs for cited player_local_condition evidence; close as an adventure handoff that leaves the player with choices without printing the whole route menu.",
    ...
    "Sentence-plan authority: write accepted_evidence sentence objects from cited sentencePlan materialObligations and claimFocus when they fit the page flow. Any promptInput.acceptedEvidence entry may be cited when the sentence states that evidence's fact, but sentencePlanRefs narrow the sentence job: when a sentence lists sentencePlanRefs, its backendFactRefs should come from those steps' materialObligations unless another cited step is also named. sentencePlan organizes prose and does not add world truth.",
    ...
    "Player-facing diction: avoid backend framing phrases such as 'current scene', 'current visible', 'current observation', or 'visible entry'. Say 'here', 'at <scene label>', 'in view', or name the accepted scene label instead. For bounded negative/local-observation lines, do not close with 'in the current scene'; use 'here', 'at <scene label>', or omit the placement when the sentence already has a scene anchor.",
    "Reference transformation examples are patterns, not stock prose. Example movement: prompt-safe accepted facts with roles `travel_beat`, `destination_label`, and `elapsed_travel_time` expose values 'North Hall comes into view after 1 minute.', 'North Hall', and '1 minute'; they may become 'North Hall comes into view after a minute.', 'A minute later, North Hall is ahead.', or 'The way spills into North Hall after a minute.' Choose the form that best fits pageVariation.openingDoor and do not reuse a distinctive arrival metaphor across nearby turns.",
    ...
    "Example route options: accepted route labels 'Anchor Chain Pylon' and 'The Copper Tap' with one-minute costs can become 'Lowwater Bazaar opens into a couple of usable ways onward.' with movement_option refs only; the structured choices carry the exact labels outside prose. If the current sentence is explicitly about one named route, preserve that route label exactly. If timing is part of the sentence, use the exact accepted cost, for example 'each takes 1 minute', and mark hardClaims with route_choice_travel_costs.",
    ...
    "Movement surface: for player_location_change, render the accepted destination and elapsed travel time as a compact arrival beat, with `destination_label`, `elapsed_travel_time`, and `current_place_after_movement` values as proof details. Movement arrival owns the page center; scene_texture is proof context unless sentencePlan explicitly supplies exact_context_texture. Do not open normal movement pages with a copied static location-card sentence or receipt wording. Prefer concise varied movement results such as '<destination> comes into view after <time>', 'The way spills into <destination> after <time>', or '<destination> catches up around you after <time>'. Avoid the stock opener 'After <time>, you reach <destination>' unless nearby movement pages used richer shapes and the page needs a bare landing. Arrival phrasing should use reach, arrive, come into view, spill into, or put you at; scene_placement/current_scene facts such as 'You are at <destination>' are context anchors, not the movement result sentence. Movement result sentences cite the player_location_change evidence and its travel_beat/destination/time facts; cite scene_placement only in a separate context sentence when sentencePlan gives that sentence its own context_anchor. Posture framing such as 'you stand at/in <destination>' belongs only to accepted player_local_condition evidence. Route safety, arrival discoveries, scenery beyond cited accepted texture in sentencePlan, encounter details, and travel-mode detail require their own accepted evidence.",
    ...
    "Route-options surface: for movement_option and route_options_receipt, treat accepted route labels as structured action handles. The narrator's prose should usually summarize the visible opportunity to choose a way onward from the route origin without printing the full route menu. Route choice travel costs remain exact hard facts when stated, but they are optional player-facing timing detail on ordinary route-handle pages. If sentencePlan supplies an optional texture sentence and recentSurfaceAvoid shows that same opening shape, spend the page on a compact choice-pressure beat instead. If texture remains useful, keep it in its own sentence and keep the route-choice beat focused on the current place opening into choices. Name one route label only when the player explicitly asked about that route or route_status owns that single route. Do not add travel mode, player motion, hidden routes, route safety, or current-scene change.",
    ...
    "For local_observation, express the accepted current visible observation result; for bounded_visibility_negative, write a playable sensory result that preserves the checked query as not settled by the current material. For whether-shaped observation_query, start from a concrete present surface or sound before the narrow non-answer, while preserving the query words as checked material.",
...
export function validateCleanNarrationCandidate(input: {
  ...
  const routeLabelsMustAppearInProse =
    promptInput.narrativePageTask.choicePresentation.labelHandling === "preserve_route_labels_verbatim";
  ...
  candidate.sentences.forEach((sentence, index) => {
    ...
    if (sentence.kind === "accepted_evidence") {
      ...
      const sentenceUsesRouteChoiceMaterial = routeLabelsMustAppearInProse && (
        sentence.claimKinds.includes("movement_option")
  ...
  const coveredRequiredEntryRefs = new Set<string>();
  for (const sentence of candidate.sentences) {
    if (sentence.kind !== "accepted_evidence") continue;
    for (const evidenceRef of sentence.evidenceRefs) {
      coveredRequiredEntryRefs.add(evidenceRef);
    }
  }
  const requiredEntryRefs = uniqueStrings(promptInput.narrativePageTask.moves
    .filter((move) =>
      move.coverage === "required"
      && move.proseMove === "render_authoritative_turn_event"
      && move.entryRefs.length > 1
    )
    .flatMap((move) => move.entryRefs));
  for (const entryRef of requiredEntryRefs) {
    if (!coveredRequiredEntryRefs.has(entryRef)) {
      issues.push({
        code: "sentence_plan_not_supported",
        path: "sentences.evidenceRefs",
        message: `Narration candidate did not cover required accepted evidence ${entryRef}.`,
      });
    }
  }
...
function isStandaloneDeviceSurfaceUnavailableView(view: CleanNarratorView): boolean {
  const terminalEvidence = view.acceptedEvidence.filter(isLiteraryTerminalEvidence);
  return terminalEvidence.length === 1
    && terminalEvidence[0]!.authority === "device_surface_observation_receipt"
    && terminalEvidence[0]!.claimKinds.includes("device_surface_observation")
    && terminalEvidence[0]!.claimKinds.includes("device_surface_unavailable");
}
...
function needsTypedHardResultRender(view: CleanNarratorView): boolean {
  return isStandaloneDeviceSurfaceUnavailableView(view);
}

function renderTypedHardResult(view: CleanNarratorView): string {
  const terminalEvidence = view.acceptedEvidence.filter(isLiteraryTerminalEvidence);
  const deviceSurfaceObservation = terminalEvidence.find((evidence) =>
    evidence.authority === "device_surface_observation_receipt"
    && evidence.claimKinds.includes("device_surface_unavailable")
  );
  if (deviceSurfaceObservation) {
    return renderDeviceSurfaceProjection(deviceSurfaceObservation);
  }
  throw new Error("Typed hard-result render requires a supported exact hard-result evidence shape.");
}
...
export async function runCleanNarration(input: {
  ...
  if (needsTypedHardResultRender(input.narratorView)) {
    const result = assertCleanNarrationResult({
      version: "gameplay-runtime.clean-narration-result.v1",
      packetId: input.narratorView.packetId,
      turnId: input.narratorView.turnId,
      text: renderTypedHardResult(input.narratorView),
      source: "typed_hard_result",
    });
    return {
      ...result,
      validationIssues: [],
