...
export function buildCleanNarrationSystemPrompt(
  ...
  return [
    ...
    "For local_observation, express the accepted current visible observation result; for bounded_visibility_negative, write a playable sensory result that preserves the checked query as unresolved in the current scene. For whether-shaped observation_query, phrase the scene as giving texture without resolving the asked direction, state, source, safety, or meaning, while preserving the query words as checked material.",
...
function renderTypedHardResult(view: CleanNarratorView): string {
  ...
  if (deviceSurfaceObservation) {
    return [
      renderSceneTextureProjection(view),
      renderDeviceSurfaceProjection(deviceSurfaceObservation),
    ].filter((text): text is string => Boolean(text && normalizeText(text).length > 0)).join(" ");
  }
