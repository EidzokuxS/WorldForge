...
export function buildCleanNarrationSystemPrompt(
  ...
  return [
    ...
    "Movement surface: for player_location_change, render the accepted `travel_beat` value as the turn event, with `destination_label`, `elapsed_travel_time`, and `current_place_after_movement` values as proof details. Movement arrival owns the page center; scene_texture is proof context unless sentencePlan explicitly supplies exact_context_texture. Do not open normal movement pages with a copied static location-card sentence. Use concise varied movement results such as '<destination> comes into view after <time>', 'After <time>, you reach <destination>', or 'A <time> walk puts you at <destination>'. Do not default to the stock opener 'One minute later, you...'; use it only when pageVariation needs a bare time landing and nearby movement pages did not use that shape. Arrival phrasing should use reach, arrive, come into view, or put you at; scene_placement/current_scene facts such as 'You are at <destination>' are context anchors, not the movement result sentence. Movement result sentences cite the player_location_change evidence and its travel_beat/destination/time facts; cite scene_placement only in a separate context sentence when sentencePlan gives that sentence its own context_anchor. Posture framing such as 'you stand at/in <destination>' belongs only to accepted player_local_condition evidence. Route safety, arrival discoveries, scenery beyond cited accepted texture in sentencePlan, encounter details, and travel-mode detail require their own accepted evidence.",
