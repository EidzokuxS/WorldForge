...
export function buildCleanNarrationSystemPrompt(
  ...
  return [
    ...
    "Local-observation surface: for local_observation, phrase only the accepted current observation entries. If sentencePlan supplies a texture sentence, keep texture there. When proseMaterials includes observed_inventory_item_labels, make the accepted item labels the sentence subject and express only their with-player state, such as '<labels> are with you' or '<label> is with you'; add scene placement only when sentencePlan also supplies scene material. Do not phrase inventory as visible scenery. When proseAssembly.sentenceShape=local_observation_line and materialWeaveOrder=observed_labels_then_scene, use observed_entry_labels plus anchor_scene as a compact scene observation such as '<label> is in sight at <scene>' or '<labels> are visible at <scene>'; preserve exact labels and scene tokens. For materialWeaveOrder=query_scene_then_bounded_no_match_proof, use observation_query plus anchor_scene as the surface and local_observation_beat as proof material for the bounded visible no-match; phrase the result as the checked query not standing out in the visible scene. Player posture, motion, grip, search action, surface-kind wording, actor action, dialogue, relationship, private knowledge, route truth, item state change, movement, broad absence, no-change, and ambient setting detail require exact accepted backendFacts; bounded_visibility_negative may only say the checked visible entries showed no matching visible result.",
...
function narrationValidationRepairLines(
  ...
  if (issues.some((issue) =>
    ...
    const routeHandoffLabels = choicePresentation.choices.map((choice) => choice.label);
    const routeHandoffRepairText = routeHandoffLabels.length === 1
      ? `${routeHandoffLabels[0]} is the way onward here.`
      : routeHandoffLabels.length > 1
      ? `${routeHandoffLabels.join(", ")} are the ways onward here.`
      : null;
    const sentencePlanSlots = promptInput.narrativePageTask.sentencePlan
      .map((step) =>
        `${step.sentenceRef} ${step.beatObjective} refs=${step.materialObligations.allowedMaterialFactRefs.join(",")}`
      )
      .join(" | ");
    ...
    if (issues.some((issue) => issue.message.includes("Direct-scene inventory sentence-plan refs"))) {
      lines.push("For direct-scene repair, split visible actor/target material, inventory_status material, and route handoff material into separate accepted_evidence sentences; each sentence cites only its own sentencePlanRef and materialObligations refs.");
    }
    if (issues.some((issue) => issue.message.includes("Direct-scene route handoff sentence used scene/place label"))) {
    ...
    if (issues.some((issue) =>
      ...
      || issue.message.includes("outside its cited sentence-plan material obligations")
      || issue.message.includes("outside its cited sentence-plan claim focus")
    )) {
      lines.push(`Repair by sentence slots only: ${sentencePlanSlots}. Remove player-facing sentence material that is absent from the cited slot refs.`);
    }
