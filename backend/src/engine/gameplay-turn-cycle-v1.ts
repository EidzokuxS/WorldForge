...
export function buildNarratorPromptFromSettledPacketV1(packet: SettledTurnPacketV1): {
  ...
  const system = [
    ...
    "For any start_search acceptedEvidence with found=false, including visual searches for signs, storefronts, landmarks, people, routes, marks, addresses, contents, or documents, never state that the searched thing is absent, not visible, not present, not found, missing, unavailable, or not giving itself to the player's gaze. Narrate only that the player begins or continues the search and that the concrete result remains unconfirmed unless another accepted receipt confirms presence or absence.",
    ...
    "Candidate lookup acceptedEvidence, including find_poi_candidates and find_location_candidates, is not local-navigation progress. Never narrate walking along directions, turning at landmarks, scanning down blocks, passing storefronts, reaching corners, or continuing through streets from lookup evidence alone; that requires accepted log_event/start_search/reveal_location/create_minor_poi/movement evidence for the same target.",
    ...
    prompt: JSON.stringify({
      contract: {
        ...
        subject: {
          ...
          playerActionUsage: "playerAction is request/framing, not settlement evidence; do not narrate unsatisfied physical micro-actions from it",
          ...
          nonPlayerPresence: "gmRead targetRefs/evidenceRefs/narrationGuardrails never establish NPC presence or visibility; require acceptedEvidence from actor lookup, visible affordance, or actor/local consequence",
        },
        forbidden: [
          "new consequences",
  ...
  switch (toolName) {
    ...
    case "record_dialogue_outcome":
      return {
        ...
        roles: descriptor.roles,
        input: {
          speakerRef: "exact visible actor/model-safe ref; required for visible speaker outcomes; omit only for unavailable/no_current_answer",
          addresseeRefs: ["Player"],
          outcomeKind: "answered|refused|silent|gestured|warned|redirected|unavailable|no_current_answer; never use unavailable/no_current_answer with speakerRef or a visible speaker",
          topicKind: "social|procedure|permission|proof|route|safety|trade|status|other",
          authorityKind: "role_authority|public_service|witness|hearsay|not_authorized|no_visible_authority|unknown; no_visible_authority only with unavailable/no_current_answer and no speakerRef",
          ...
          futureRelevance: "required when durable",
          requestedRoleText: "optional non-empty string; required only for unavailable/no_current_answer and GM Read prose_role/no_visible_authority bindings; omit otherwise, never empty string; unavailable/no_current_answer must also omit speakerRef",
          quote: "direct speech when answered/warned/redirected and durable; every outcome needs quote or claims",
          ...
          claims: [{
            ...
            polarity: "allows|denies|requires|redirects|unknown|states",
            subjectText: "free text when not an exact visible ref",
            summary: "claim summary; required when quote is omitted, including unavailable/no_current_answer",
          }],
          stateEffects: [{
...
export function gmActionChecklistSystemPromptV1(): string {
  return [
    ...
    "If the player follows speaker-provided street directions toward a named POI/micro-location that is not an exposed connected destination in the SceneFrame, set targetBinding.targetKind=unmodeled_poi_or_micro_location and movementAuthority=none; do not use move_actor to a different nearby/known route. Use scene-local log_event for the attempted following-directions/local-navigation beat plus start_search/find_poi_candidates/find_location_candidates/create_minor_poi/reveal_location as appropriate to establish the target before any current-scene change. A candidate lookup alone does not own walking progress along the directions.",
    ...
    "If the player asks who/which people/actors/NPCs/guards/traders/porters are visible, nearby, or present, include a required list_visible_affordances or find_actor_candidates step. Never satisfy a visible-person request with routes, points of interest, objects, or gmRead refs alone.",
    "If the player asks for a mixed current-scene affordance such as visible people plus routes, signs, objects, hazards, or points of interest, use list_visible_affordances or separate matching find_actor_candidates/find_object_candidates/find_poi_candidates/list_navigation_options steps covering every requested category.",
    ...
    "A start_search step records that the search target is unconfirmed; expectedVisibleEffect must say the search starts/continues and the concrete result remains unconfirmed. It must not assert the searched detail exists or is absent, and must not say it is found, not present, not visible, or that none were present.",
