...
function formatGmToolLoopProfilePrompt(profile: GmToolLoopProfile): string {
  ...
  if (profile.name === "procedural_conversation_outcome") {
    return [
      ...
      "Treat the speaker's profile, traits, goals, orders, and private knowledge as decision context, not dialogue content.",
      "A player question creates an opportunity to respond, not a reason to disclose. Decide what the speaker intentionally reveals from their goals, relationship, trust, risk, leverage, and current pressure.",
      "Do not volunteer hidden objectives, orders, stakes, names, allies, destinations, or failure conditions merely because the speaker knows them. Prefer a bounded answer, refusal, deflection, warning, lie, or counter-question when disclosure would work against the speaker; disclose only when the speaker has a clear in-scene reason.",
      "For record_dialogue_outcome, semantics live in outcomeKind/topicKind/authorityKind/truthStatus/futureUseKind, claims, and stateEffects. Every outcome needs quote or at least one structured claim; summary is support/display only and is never final-narration authority. Durable answered procedure/permission/proof/route/safety/status outcomes require quote. If no concrete answer can be spoken, use refused/unavailable/no_current_answer/redirected instead of answered.",
...
export function buildGmToolLoopPrompt(
  ...
  return [
    ...
    "- record_dialogue_outcome must include quote or structured claims; summary alone is support/display text and cannot ground final narration.",
