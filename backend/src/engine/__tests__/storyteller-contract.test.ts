...
describe("storyteller-contract", () => {
  ...
  it("uses a non-conflicting final-visible grounded sentence draft output mode", () => {
    ...
    expect(contract).toContain("sentences[].text with player-visible prose plus evidenceRefs");
    expect(contract).toContain("sentences[].evidenceRefs array must contain 1-4 short packet evidence refs");
    expect(contract).toContain("Do not output sentences[].factRefs");
    expect(contract).toContain("evidenceRefs prove hard facts");
    expect(contract).toContain("never return five or more evidenceRefs on one sentence");
    expect(contract).not.toContain("sentences[].factRefs with exactly one backendFacts ref");
    expect(contract).toContain("apply inside sentences[].text");
