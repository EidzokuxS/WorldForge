...
describe("clean Stage 6 narration contracts", () => {
  ...
  it("accepts missing scene_texture for item_state at runtime without prose-quality repair", async () => {
    ...
    const result = await runCleanNarration({
      ...
      generateCandidate: async (request) => {
        ...
        expect(request.prompt).not.toContain("Stage 6 validation feedback");
        return acceptedCandidate(view, [{
          text: "Brass Tube changes hands from Player to Guide at Market, and Guide now carries Brass Tube there.",
          evidenceRefs: ["e1"],
    ...
    expect(attempts).toBe(1);
    expect(result.source).toBe("model");
    expect(result.text).toBe("Brass Tube changes hands from Player to Guide at Market, and Guide now carries Brass Tube there.");
  });
  ...
  it("accepts direct-scene custody status at runtime without prose-quality repair", async () => {
    ...
    expect(attempts).toBe(1);
    expect(result.source).toBe("model");
    expect(result.text).toBe("Canvas awnings hang over the market lanes. Guide is in view, with Brass Tube and Notice Board visible nearby. Courier satchel is in your keeping. North Hall is the way onward from here.");
  });
  ...
  it("uses model-authored literary narration for direct scene targets and exits", async () => {
    ...
    expect(result.source).toBe("model");
    expect(result.text).toBe("Canvas awnings hang over the market lanes. Notice Board is visible. Courier satchel is in your keeping. North Hall is the way onward from here.");
  ...
  it("uses accepted scene_texture for item_state prose when texture is available", async () => {
    ...
    expect(result.source).toBe("model");
    expect(result.text).toBe("Rain taps the brass gutters. Brass Tube changes hands from Player to Guide at Market, and Guide now carries Brass Tube there.");
  ...
  it("uses accepted scene_texture for composed item_state plus dialogue_response without expanding the quote into truth", async () => {
    ...
    expect(result.source).toBe("model");
    expect(result.text).toBe('Rain taps the brass gutters. Brass Tube changes hands from Player to Guide at Market, and Guide now carries Brass Tube there. Guide answers: "The north stairs flooded before dawn."');
  ...
  it("uses model-authored direct-scene prose for scene_observation receipts with direct-scene guards", async () => {
    ...
    expect(modelCalls).toBe(1);
    expect(result.source).toBe("model");
    expect(result.text).toBe("At Market, Guide is in view. Courier satchel is in your keeping. North Hall is the way onward from here.");
    ...
    expect(texturedResult.text).toBe("Canvas awnings hang over the market lanes. At Market, Guide is in view. Courier satchel is in your keeping. North Hall is the way onward from here.");
  });
  ...
  it("accepts receipt-shaped wording according to structured refs rather than marker scanning", () => {
    ...
    const result = validateCleanNarrationCandidate({
      view,
      candidate: acceptedCandidate(view, [{
        text: "Guide now carries Brass Tube at Market. Target: Guide. Final equip state: carried. Current scene anchor: Market.",
        evidenceRefs: ["e1"],
        backendFactRefs: ["e1.f2", "e1.f1", "e1.f3", "e1.f5", "e1.f6", "e1.f7"],
        claimKinds: ["item_state"],
      }]),
