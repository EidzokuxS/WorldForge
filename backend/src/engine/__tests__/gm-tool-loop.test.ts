...
describe("runGmToolLoop", () => {
  ...
  it.each([
    ...
    {
      label: "chronicle entry records a durable public notice",
      allowedTools: ["add_chronicle_entry"],
      text:
        "The public docket gains a durable notice that the afternoon appeal window has opened.",
      toolCall: {
        toolName: "add_chronicle_entry",
        input: {
          text: "The afternoon appeal window opens on the public docket.",
          importance: 6,
          participants: ["Player"],
        },
      },
      toolResult: {
        success: true,
        result: { entryId: "chronicle-appeal-window", persisted: true },
        authority: toolAuthority(["chronicle:chronicle-appeal-window"]),
      },
      runtimeRequirement: {
        kind: "scene_beat",
        durability: "durable",
