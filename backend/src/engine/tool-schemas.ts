...
function toJsonToolValue(value: unknown): JSONValue {
  return JSON.parse(JSON.stringify(value ?? null)) as JSONValue;
}

type StorytellerToolModelOutput = {
  type: "json";
  value: JSONValue;
};

function toStorytellerModelOutput({ output }: { output: unknown }): StorytellerToolModelOutput {
  const value = output && typeof output === "object" && "success" in output
    ? toModelVisibleToolResult(output as ToolResult)
    : output;
  return {
    type: "json",
    value: toJsonToolValue(value),
  };
}

type StorytellerToolDefinition = {
  description: string;
  inputSchema: unknown;
  execute: (
...
function storytellerTool(
  definition: StorytellerToolDefinition,
): Tool<Record<string, unknown>, ToolResult> {
  return tool<Record<string, unknown>, ToolResult>({
    ...definition,
    inputSchema: definition.inputSchema as FlexibleSchema<Record<string, unknown>>,
    toModelOutput: toStorytellerModelOutput,
  });
}
...
export function createStorytellerTools(
  ...
  return {
    list_visible_affordances: storytellerTool({
      description:
    ...
    list_navigation_options: storytellerTool({
      description:
    ...
    find_location_candidates: storytellerTool({
      description:
    ...
    find_object_candidates: storytellerTool({
      description:
    ...
    find_actor_candidates: storytellerTool({
      description:
    ...
    find_poi_candidates: storytellerTool({
      description:
    ...
    inspect_known_fact: storytellerTool({
      description:
    ...
    move_actor: storytellerTool({
      description:
        "State-bearing bridge. Move only the current player/subject actor along a legal movement candidate backed by prompt aliases such as current_scene/current_location, move_*, or route/check_route helper aliases; returns destination, path, travel cost, and actor refs.",
      inputSchema: moveActorInputSchema,
      execute: (args) => executeRuntimeTool("move_actor", args),
    }),

    create_minor_poi: storytellerTool({
      description:
    ...
    create_scene_extra: storytellerTool({
      description:
    ...
    record_player_intent: storytellerTool({
      description:
    ...
    record_dialogue_outcome: storytellerTool({
      description:
    ...
    record_world_fact: storytellerTool({
      description:
        "State-bearing semantic world fact. Use when the player compares, verifies, or records future-usable public/known facts without an NPC dialogue outcome. Semantics live in sourceKind/truthStatus/factKind/topicKind/claims; summary may be any language and is display/evidence only. claims[].subjectRef, subjectRefs, and sourceRefs use prompt aliases such as current_scene/current_location or helper aliases only; put ordinary display names like notice board or route-log mismatch in subjectText.",
      inputSchema: recordWorldFactInputSchema,
      execute: (args) => executeRuntimeTool("record_world_fact", args),
    }),

    add_tag: storytellerTool({
      description:
    ...
    set_relationship: storytellerTool({
      description:
    ...
    add_chronicle_entry: storytellerTool({
      description:
    ...
    log_event: storytellerTool({
      description:
        "Log a scene-local player-turn beat: attempted, refused, witnessed, conversational, sensory/non-durable, or bluff beats. Future-usable player-known facts must use record_world_fact, record_dialogue_outcome, or the concrete state tool that owns the change.",
      inputSchema: logEventInputSchema,
      execute: (args) => executeRuntimeTool("log_event", args),
    }),

    advance_time: storytellerTool({
      description:
    ...
    offer_quick_actions: storytellerTool({
      description:
    ...
    reveal_location: storytellerTool({
      description:
    ...
    request_contested_outcome: storytellerTool({
      description:
