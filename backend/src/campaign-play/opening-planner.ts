...
function openingSemanticReviewPrompt(
  ...
  return [
    ...
    "The playerPremise.summary is the source interaction claim that Opening narration will present, while decision.summary and its branch labels are the visible decision authority. Compare them field by field before accepting: one concrete subject, every material state or qualifier, the directing actor, destination, payment, deadline, and consequence must stay the same. A qualifier changes the subject for this review; for example, unstamped consignment chits and stamped consignment chits are different things, even when the noun is similar. A branch label may be concise or generic, but it must never name a different subject or qualifier from the interaction claim.",
    ...
    "No available opening acceptance effect types a later privilege or consequential return beyond its typed fields. A paid_delivery fee with on_completion timing is a supported current deal term; reject any additional or untyped payment. Also reject a definite promise of steadier or preferred future work, access, permission, endorsement, recommendation, vouching, relationship change, service, reward, or another future benefit when it is attached to accepting this decision. The proposal may mention an unsupported possibility only as clearly non-binding atmosphere; it must not present that possibility as a promised consequence, and it must not make the current choice rely on it.",
  ...
  return {
    ...
    async plan(request) {
      ...
      const recoveryPrompt = (
        ...
        "Recovery must align playerPremise.summary, decision.summary, acceptLabel, and declineLabel on one concrete subject and every material state or qualifier, with the same actor, destination, payment, deadline, and typed consequence. If a label or narration claim names a different qualifier, correct the proposal itself rather than hiding the mismatch.",
        "If the rejection identifies a future privilege or other consequence without a matching typed acceptance effect, or beyond a typed paid_delivery fee, remove that promise or state only a clearly non-binding possibility; do not invent a new effect, and do not leave the current decision dependent on it.",
