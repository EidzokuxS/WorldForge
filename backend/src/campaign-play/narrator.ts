...
actorProfiles is private identity and grammatical authority for visibleActors. Preserve each person's established identity and pronouns. Use a profile only to identify that person and refer to them consistently; it is not a source of scene facts, motive, mood, knowledge, action, or disclosure. Never state a profile claim unless another allowed source above independently supports it in the current visible scene.
