...
function prompt(frame: CampaignPlayJudgeFrame, input: CampaignPlayJudgeInput): string {
  ...
  return [
    ...
    "A persistent location is the complete current scene, not a container for model-authored rooms, corridors, branches, floors, or other destinations. A freeform request to enter or explore beyond that scene is actionable only through one exact VISIBLE_ROUTES route. Without that route, use clarification_required and ask which visible destination the player means. Do not reinterpret unmodeled traversal as a local attempt.",
    ...
    "A positive possession entry in VISIBLE_FRAME is the only authority that the player currently controls a tool or material. A general tool possession authorizes only the tools it names; it never includes raw material, fasteners, ammunition, medicine, food, fuel, currency, or another consumable. A work assignment, posted supply list, visible stock, offer, request, dialogue, handling, transport, or narration does not issue supplies to the player. If PLAYER_INPUT directly uses a tool or consumable that has no visible possession handle, classify it as impossible and identify the missing material basis in reason; do not add that material to method or stakes. A request to a targeted present actor for that item is contact, not direct use: authorize a permitted acquire instead of assuming either transfer or refusal. When a visible possession is consumed or materially changed, possessionEffectAuthority must use required spend or transform with that exact cited handle.",
