...
player.motivations is the ordered list from the player's CharacterRecord. ${toolMode
    ? "When it is empty, use the no-motivation transport state. Otherwise choose one motivation by its zero-based motivationIndex."
...
Return exactly one strict object with top-level keys start, scene, playerPremise. In tool mode, playerPremise is exactly {"state":"none"} when player.motivations is empty. Otherwise it is exactly {"state":"motivated","motivationIndex":number,"anchor":"openingActor"|"supportActor","eventClass":"dialogue"|"interaction","summary":string,"routeRestriction":{"state":"none"|"restricted","reason":string}}. Use an empty reason only with state none; use one short concrete non-empty reason only with state restricted. Do not return null for playerPremise or routeRestriction in tool mode.` : ""}`;
}
