...
Choose the hidden consequence source only from openingConstraints.plannedActors. Copy its actorId and one actorLocationId from that entry. Call the chosen scene candidate selectedScene. The hidden location must differ from selectedScene.locationId. The compiler takes the hidden goal and observable trace from that actor's selected primary goal and first plan step. Omit goalId and observableTrace from hiddenConsequence. Bind the hidden actor's first step to the exposure with one exact target:
