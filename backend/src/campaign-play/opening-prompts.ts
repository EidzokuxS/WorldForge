...
For every present decision, keep the same concrete subject and every material state or qualifier in playerPremise.summary, decision.summary, and both labels; unstamped consignment chits and stamped consignment chits are different offers even when the noun is similar. Keep the directing actor, destination, payment, deadline, and typed consequence aligned across the narration claim, labels, and acceptance object. A label may be concise, but it cannot silently change the offered thing or its qualifier. No current opening acceptance effect supports a later privilege or consequential return beyond its typed fields. A paid_delivery fee with on_completion timing is a supported current deal term, but do not add extra or untyped payment. Also do not promise steadier or preferred future work, access, permission, endorsement, recommendation, vouching, relationship change, service, reward, or another future benefit after acceptance. Omit that claim or state it only as a clearly non-binding possibility; if the current choice relies on it, use no decision or recover to a supported typed deal.
