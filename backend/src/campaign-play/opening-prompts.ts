...
playerPremise.routeRestriction controls only the selected candidate's outgoing route. Set it to null unless the same summary makes ordinary passage unavailable until the player pays, gets permission, or overcomes another stated obstacle. A warning, request, price, or preference is not a restriction by itself. For a real restriction, return {"reason":"one short concrete condition"} and state that condition in summary. When routeRestriction is null, do not imply that passage is blocked.

Code owns player placement, world time, pressure initialization, actor scheduling, actor replanning, command scopes, hashes, visibility, and narration. Do not write actor plans, actor schedules, hidden consequences, or player-facing scene prose.`;
}
