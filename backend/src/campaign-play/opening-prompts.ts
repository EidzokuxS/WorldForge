...
- For route_state, exposure contains exactly channel and triggers. Set triggers to a unique array of one to three exact literals chosen only from "inspect", "attempt", and "traverse". The compiler uses selectedScene.routeId; include {"kind":"route","id":selectedScene.routeId} in the first step targets.
