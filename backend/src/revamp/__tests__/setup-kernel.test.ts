...
import { createRevampStartingSetup } from "../setup-kernel.js";
...
describe("revamp setup kernel", () => {
  ...
  it("persists starting setup and advances to setup_ready", () => {
    ...
    const kernel = createRevampStartingSetup({ campaignId: CAMPAIGN_ID });
  ...
  it("fails before cast_ready", () => {
    ...
    expect(() => createRevampStartingSetup({ campaignId: CAMPAIGN_ID })).toThrow(
