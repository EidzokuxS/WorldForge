...
describe("revamp locations adapter", () => {
  it("maps scaffold locations into revamp graph nodes and spatial edges", () => {
  ...
  it("does not import old route or generator implementations", () => {
    const source = fs.readFileSync(
      path.resolve(process.cwd(), "src/revamp/locations-adapter.ts"),
      "utf-8",
    );
