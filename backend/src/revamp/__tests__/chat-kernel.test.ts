...
import { createRevampChatMessage } from "../chat-kernel.js";
...
describe("revamp chat kernel", () => {
  ...
  it("persists user and assistant turns and advances turn index", () => {
    ...
    const result = createRevampChatMessage({
  ...
  it("fails before active", () => {
    ...
    expect(() =>
      createRevampChatMessage({ campaignId: CAMPAIGN_ID, message: "Look around" }),
  ...
  it("fails on an empty direct message", () => {
    ...
    expect(() =>
      createRevampChatMessage({ campaignId: CAMPAIGN_ID, message: "   " }),
