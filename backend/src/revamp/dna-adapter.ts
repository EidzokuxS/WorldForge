...
export const REVAMP_KERNEL_FILE_NAME = "kernel.json";
...
const WORLD_DNA_LABELS: Record<keyof RevampWorldDNA, string> = {
  geography: "geography",
...
export function getRevampCampaignKernelPath(campaignId: string): string {
  assertSafeId(campaignId);
  return path.join(getCampaignDir(campaignId), REVAMP_KERNEL_FILE_NAME);
}
...
export function buildRevampWorldDna(seeds: Partial<WorldSeeds> | undefined): RevampWorldDNA {
  return {
...
export function readCampaignKernel(campaignId: string): CampaignKernel | null {
  const kernelPath = getRevampCampaignKernelPath(campaignId);
...
export function writeCampaignKernel(campaignId: string, kernel: CampaignKernel): void {
  ...
  fs.writeFileSync(getRevampCampaignKernelPath(campaignId), JSON.stringify(kernel, null, 2), "utf-8");
}
...
export function advanceCampaignKernelToWorldReady(campaignId: string): CampaignKernel {
  ...
  const worldDna = buildRevampWorldDna(config.seeds);
