...
function allCastMembers(castRegistry: RevampCastRegistry): RevampCastMember[] {
  return [
...
function assertUniqueBaseIds(graph: RevampWorldGraph): void {
...
function createCharacterNode(member: RevampCastMember): RevampWorldNode {
  return {
    id: member.id,
    type: "Character",
...
export function buildRevampWorldGraph(input: {
  baseGraph: RevampWorldGraph;
