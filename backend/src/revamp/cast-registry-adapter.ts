...
export type RevampPlayerCastSource = "player_created" | "player_imported";
export type RevampNpcCastSource = "npc_imported" | "npc_generated";

export interface RevampCastPlacementInput {
  locationId?: string | null;
  sceneLocationId?: string | null;
  notes?: readonly string[];
}

export interface RevampDraftCastInput {
  draft: CharacterDraft;
  source: RevampNpcCastSource;
  campaignRole?: RevampCampaignRole;
  importance?: RevampCastImportance;
  placement?: RevampCastPlacementInput;
}

export interface RevampScaffoldNpcCastInput {
  npc: ScaffoldNpc;
  source: RevampNpcCastSource;
  campaignRole?: RevampCampaignRole;
  importance?: RevampCastImportance;
...
export interface BuildRevampCastRegistryInput {
  playerCharacter?: {
    draft: CharacterDraft;
    source?: RevampPlayerCastSource;
    placement?: RevampCastPlacementInput;
  } | null;
  importedCast?: readonly (RevampDraftCastInput | RevampScaffoldNpcCastInput)[];
  generatedCast?: readonly (RevampDraftCastInput | RevampScaffoldNpcCastInput)[];
}
...
function castMemberId(source: RevampCastSource, name: string): string {
  const readable = name
    .normalize("NFKD")
    .replace(/[\u0300-\u036f]/g, "")
...
function resolvePlacement(
  draft: CharacterDraft,
  placement: RevampCastPlacementInput | undefined,
): RevampCastPlacement {
...
function defaultNpcRole(draft: CharacterDraft): RevampCampaignRole {
...
function defaultNpcImportance(draft: CharacterDraft): RevampCastImportance {
  if (draft.identity.tier === "key") {
    return "major";
  }
  if (draft.identity.tier === "temporary") {
...
function normalizeScaffoldNpcDraft(
  input: RevampScaffoldNpcCastInput,
): CharacterDraft {
  if (input.npc.draft) {
...
function isScaffoldNpcInput(
  input: RevampDraftCastInput | RevampScaffoldNpcCastInput,
): input is RevampScaffoldNpcCastInput {
  return "npc" in input;
}
...
function createCastMember(
  ...
  let scaffoldInput: RevampScaffoldNpcCastInput | null = null;
...
function createPlayerMember(
  input: NonNullable<BuildRevampCastRegistryInput["playerCharacter"]>,
): RevampCastMember {
...
function assertUniqueCastNames(members: readonly RevampCastMember[]): void {
...
export function buildRevampCastRegistry(
  input: BuildRevampCastRegistryInput,
): RevampCastRegistry {
