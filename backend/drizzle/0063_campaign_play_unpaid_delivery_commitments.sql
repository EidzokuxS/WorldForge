...
--> statement-breakpoint
DROP TRIGGER campaign_play_commitments_delete_immutable;
--> statement-breakpoint
DROP TRIGGER campaign_play_events_commitment_ref_guard;
--> statement-breakpoint
DROP TRIGGER campaign_play_decisions_accept_effect_insert_guard;
--> statement-breakpoint

ALTER TABLE campaign_play_commitments RENAME TO __campaign_play_commitments_0063_old;
--> statement-breakpoint
...
CREATE TABLE campaign_play_commitments (
  ...
  CONSTRAINT campaign_play_commitments_payload_valid CHECK (
    ...
    AND kind IN ('paid_delivery', 'unpaid_delivery')
    ...
    AND (
      ...
      OR (kind = 'unpaid_delivery'
        AND fee_unit IS NULL
...
INSERT INTO campaign_play_commitments (
  ...
  source_decision_key, source_turn_id, source_receipt_id, completion_turn_id,
  completion_receipt_id, world_version, created_at, updated_at
)
SELECT commitment_id, campaign_id, performer_actor_id, counterparty_actor_id,
  kind, status, title, subject_name, destination_handle, fee_unit, fee_amount,
  payment_timing, accepted_world_time_minutes, due_world_time_minutes,
  source_decision_key, source_turn_id, source_receipt_id, completion_turn_id,
  completion_receipt_id, world_version, created_at, updated_at
FROM __campaign_play_commitments_0063_old;
--> statement-breakpoint

DROP TABLE __campaign_play_commitments_0063_old;
--> statement-breakpoint
    ...
    AND (
      (
        ...
        AND (
          ...
          OR (json_type(NEW.accept_effect_json, '$.dueInMinutes') = 'integer'
            AND json_extract(NEW.accept_effect_json, '$.dueInMinutes') BETWEEN 1 AND 10080)
        )
      ) OR (
        json_extract(NEW.accept_effect_json, '$.kind') = 'unpaid_delivery'
        AND (SELECT count(*) FROM json_each(NEW.accept_effect_json)) IN (4, 5)
        AND json_type(NEW.accept_effect_json, '$.title') = 'text'
        AND length(json_extract(NEW.accept_effect_json, '$.title')) BETWEEN 1 AND 240
        AND json_type(NEW.accept_effect_json, '$.subjectName') = 'text'
        AND length(json_extract(NEW.accept_effect_json, '$.subjectName')) BETWEEN 1 AND 120
        AND json_type(NEW.accept_effect_json, '$.destinationHandle') = 'text'
        AND length(json_extract(NEW.accept_effect_json, '$.destinationHandle')) BETWEEN 1 AND 128
        AND json_type(NEW.accept_effect_json, '$.feeUnit') IS NULL
        AND json_type(NEW.accept_effect_json, '$.feeAmount') IS NULL
        AND json_type(NEW.accept_effect_json, '$.paymentTiming') IS NULL
        AND (
          json_type(NEW.accept_effect_json, '$.dueInMinutes') IS NULL
          OR (json_type(NEW.accept_effect_json, '$.dueInMinutes') = 'integer'
            AND json_extract(NEW.accept_effect_json, '$.dueInMinutes') BETWEEN 1 AND 10080)
  ...
  SELECT CASE WHEN NOT EXISTS (
    ...
    WHERE receipt.receipt_id = NEW.completion_receipt_id
      ...
      AND (
        ...
        OR (NEW.kind = 'unpaid_delivery'
          AND NOT EXISTS (
            SELECT 1 FROM campaign_play_commands AS incur
            WHERE incur.campaign_id = NEW.campaign_id
              AND incur.batch_id = command.batch_id
              AND incur.command_kind = 'incur_actor_obligation'
          ))
...
CREATE TABLE `__campaign_play_0063_schema_guard` (
  `valid` integer NOT NULL CHECK (`valid` = 1)
);
--> statement-breakpoint
INSERT INTO `__campaign_play_0063_schema_guard` (`valid`) VALUES (
  CASE WHEN
    (SELECT count(*) FROM sqlite_schema WHERE type='table' AND name='campaign_play_commitments'
      AND instr(sql, 'unpaid_delivery')>0
      AND instr(sql, 'fee_unit TEXT')>0) = 1
    AND (SELECT count(*) FROM sqlite_schema WHERE type='trigger'
      AND name IN ('campaign_play_commitments_insert_guard', 'campaign_play_commitments_update_guard',
        'campaign_play_commitments_delete_immutable', 'campaign_play_events_commitment_ref_guard')) = 4
  THEN 1 ELSE 0 END
);
--> statement-breakpoint
DROP TABLE `__campaign_play_0063_schema_guard`;
--> statement-breakpoint
