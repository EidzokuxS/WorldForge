...
ALTER TABLE campaign_play_commitments
  ADD COLUMN destination_location_id TEXT NOT NULL DEFAULT ''
  REFERENCES locations(id) ON DELETE RESTRICT;
--> statement-breakpoint

CREATE TABLE `__campaign_play_0067_destination_backfill_guard` (
  `valid` integer NOT NULL CHECK (`valid` = 1)
);
--> statement-breakpoint
INSERT INTO `__campaign_play_0067_destination_backfill_guard` (`valid`) VALUES (
  CASE WHEN NOT EXISTS (
    SELECT 1
    FROM campaign_play_commitments AS commitment
    WHERE (
      SELECT count(*)
      FROM campaign_play_receipts AS source_receipt
      JOIN campaign_play_commands AS source_command
        ON source_command.command_id = source_receipt.command_id
      JOIN json_each(source_command.protected_payload_json, '$.affectedRefs') AS destination_ref
      WHERE source_receipt.receipt_id = commitment.source_receipt_id
        AND source_receipt.campaign_id = commitment.campaign_id
        AND source_command.campaign_id = commitment.campaign_id
        AND source_command.command_kind = 'create_player_commitment'
        AND json_extract(destination_ref.value, '$.kind') = 'location'
    ) <> 1
  ) THEN 1 ELSE 0 END
);
--> statement-breakpoint
DROP TABLE `__campaign_play_0067_destination_backfill_guard`;
--> statement-breakpoint

DROP TRIGGER campaign_play_commitments_update_guard;
--> statement-breakpoint

UPDATE campaign_play_commitments AS commitment
...
CREATE TRIGGER campaign_play_commitments_destination_insert_guard
BEFORE INSERT ON campaign_play_commitments
FOR EACH ROW
WHEN NEW.destination_location_id = '' OR NOT EXISTS (
  SELECT 1
  FROM locations AS destination
  JOIN campaign_play_receipts AS source_receipt
    ON source_receipt.receipt_id = NEW.source_receipt_id
  JOIN campaign_play_commands AS source_command
    ON source_command.command_id = source_receipt.command_id
  WHERE destination.campaign_id = NEW.campaign_id
    AND destination.id = NEW.destination_location_id
    AND source_receipt.campaign_id = NEW.campaign_id
    AND source_command.campaign_id = NEW.campaign_id
    AND source_command.command_kind = 'create_player_commitment'
    AND json_extract(source_command.protected_payload_json, '$.destinationHandle') = NEW.destination_handle
    AND json_extract(source_command.protected_payload_json, '$.destinationLocationId') = NEW.destination_location_id
    AND (SELECT count(*) FROM json_each(source_command.protected_payload_json, '$.affectedRefs')
      WHERE json_extract(value, '$.kind') = 'location') = 1
    AND EXISTS (
      SELECT 1 FROM json_each(source_command.protected_payload_json, '$.affectedRefs') AS destination_ref
      WHERE json_extract(destination_ref.value, '$.kind') = 'location'
        AND json_extract(destination_ref.value, '$.id') = NEW.destination_location_id
    )
)
BEGIN
  SELECT RAISE(ABORT, 'campaign_play_commitment_destination_invalid');
END;
--> statement-breakpoint
...
BEGIN
  ...
  SELECT CASE WHEN NOT EXISTS (
    ...
    WHERE completion_receipt.receipt_id = NEW.completion_receipt_id
      ...
      AND json_extract(completion_command.protected_payload_json, '$.destinationHandle') = NEW.destination_handle
      AND json_extract(completion_command.protected_payload_json, '$.destinationLocationId') = NEW.destination_location_id
...
CREATE TABLE `__campaign_play_0067_schema_guard` (
  `valid` integer NOT NULL CHECK (`valid` = 1)
);
--> statement-breakpoint
INSERT INTO `__campaign_play_0067_schema_guard` (`valid`) VALUES (
  CASE WHEN
    (SELECT count(*) FROM sqlite_schema
      WHERE type = 'trigger'
        ...
        AND instr(sql, 'destination_location_id') > 0
        AND instr(sql, '$.destinationLocationId') > 0
    ...
    AND (SELECT count(*) FROM pragma_table_info('campaign_play_commitments')
      WHERE name = 'destination_location_id' AND "notnull" = 1) = 1
    AND (SELECT count(*) FROM sqlite_schema
      WHERE type = 'trigger'
        AND name = 'campaign_play_commitments_destination_insert_guard'
        AND instr(sql, '$.destinationLocationId') > 0
        AND instr(sql, 'NEW.destination_location_id') > 0) = 1
  THEN 1 ELSE 0 END
);
--> statement-breakpoint
DROP TABLE `__campaign_play_0067_schema_guard`;
--> statement-breakpoint
