...
DROP TRIGGER campaign_play_actor_schedules_insert_guard;--> statement-breakpoint
DROP TRIGGER campaign_play_actor_schedules_update_guard;--> statement-breakpoint
DROP TRIGGER campaign_play_actor_schedules_delete_immutable;--> statement-breakpoint
...
CREATE TABLE `__new_campaign_play_actor_schedules` (
	...
	CONSTRAINT "campaign_play_actor_schedules_valid" CHECK("__new_campaign_play_actor_schedules"."next_act_at_world_time_minutes" BETWEEN 0 AND 2147483647
      AND ("__new_campaign_play_actor_schedules"."last_act_at_world_time_minutes" IS NULL OR "__new_campaign_play_actor_schedules"."last_act_at_world_time_minutes" BETWEEN 0 AND 2147483647)
      AND "__new_campaign_play_actor_schedules"."priority" BETWEEN 1 AND 5
      AND "__new_campaign_play_actor_schedules"."agency_debt" BETWEEN 0 AND 100)
);--> statement-breakpoint
INSERT INTO `__new_campaign_play_actor_schedules`(
  ...
  `schedule_id`, `campaign_id`, `actor_id`, `plan_id`, `next_act_at_world_time_minutes`,
  `last_act_at_world_time_minutes`, `priority`, `agency_debt`, `created_at`, `updated_at`
FROM `campaign_play_actor_schedules`;--> statement-breakpoint
...
DROP TABLE `campaign_play_actor_schedules`;--> statement-breakpoint
ALTER TABLE `__new_campaign_play_actor_schedules` RENAME TO `campaign_play_actor_schedules`;--> statement-breakpoint
...
BEGIN
  SELECT CASE WHEN NOT (
    EXISTS (
      ...
      NEW.plan_id IS NULL AND NEW.last_act_at_world_time_minutes IS NULL
      AND NEW.agency_debt = 0 AND NEW.next_act_at_world_time_minutes >= 0
      AND EXISTS (
        SELECT 1 FROM actors actor
        JOIN actor_goals goal ON goal.actor_id = actor.id AND goal.campaign_id = actor.campaign_id
        WHERE actor.id = NEW.actor_id AND actor.campaign_id = NEW.campaign_id
          AND actor.controller = 'agent' AND actor.kind = 'person' AND goal.status = 'active'
      )
    )
...
BEGIN SELECT RAISE(ABORT, 'campaign_play_actor_schedule_immutable'); END;--> statement-breakpoint
...
BEGIN
  SELECT CASE WHEN NOT EXISTS (
    ...
    WHERE turn_row.id = NEW.turn_id AND turn_row.campaign_id = NEW.campaign_id
      ...
      AND (
        NEW.admitted_plan_id IS NULL OR EXISTS (
