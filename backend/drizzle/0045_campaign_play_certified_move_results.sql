    ...
    WHERE t.id = NEW.turn_id AND t.campaign_id = NEW.campaign_id
      ...
      AND NEW.terminal_reason = CASE
        ...
        WHEN EXISTS (
          ...
          json_extract(t.input_json, '$.frame.executionRoute.kind') = 'certified_move'
          AND json_extract(t.input_json, '$.frame.executionRoute.certificate.publicResult.disposition') = 'deterministic'
          AND json_extract(t.model_selection_json, '$.routeKind') = 'certified_move'
  ...
  VALUES (
    ...
    CASE
      ...
      WHEN EXISTS (
        ...
        json_extract(NEW.input_json, '$.frame.executionRoute.kind') = 'certified_move'
        AND json_extract(NEW.input_json, '$.frame.executionRoute.certificate.publicResult.disposition') = 'deterministic'
        AND json_extract(NEW.model_selection_json, '$.routeKind') = 'certified_move'
      ) THEN 'action_resolved'
      ELSE 'invalid'
    END,
    NEW.completed_at
