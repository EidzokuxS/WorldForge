...
CREATE TABLE `__new_campaign_play_actor_jobs` (
	...
	CONSTRAINT "campaign_play_actor_jobs_valid" CHECK("__new_campaign_play_actor_jobs"."due_reason" IN ('scheduled', 'agency_debt', 'plan_retry')
      ...
      AND (("__new_campaign_play_actor_jobs"."stage" = 'deferred' AND "__new_campaign_play_actor_jobs"."defer_reason" IN ('incapacitated', 'actor_capacity', 'replan_capacity'))
...
CREATE TEMP TABLE `__campaign_play_0033_actor_job_guard` (
  `incompatible_count` integer NOT NULL CHECK (`incompatible_count` = 0)
);--> statement-breakpoint
INSERT INTO `__campaign_play_0033_actor_job_guard` (`incompatible_count`)
SELECT count(*) FROM `campaign_play_actor_jobs`
WHERE `stage` IN ('claimed', 'interrupted', 'proposed', 'deferred');--> statement-breakpoint
DROP TABLE `__campaign_play_0033_actor_job_guard`;--> statement-breakpoint
INSERT INTO `__new_campaign_play_actor_jobs`(
  ...
  "job_id", "campaign_id", "turn_id", "actor_id", "plan_id", "plan_id",
  "due_reason", "frozen_base_world_version", "worker_epoch", "claim_turn_worker_epoch",
  "stage", "proposal_id", NULL, "created_at", "completed_at"
...
CREATE TRIGGER campaign_play_actor_due_sets_capacity_guard BEFORE INSERT ON campaign_play_actor_due_sets
BEGIN
  SELECT CASE WHEN (
    SELECT count(*) FROM json_each(NEW.decisions_json) decision
    WHERE json_extract(decision.value, '$.disposition') = 'wake'
  ) > 3 THEN RAISE(ABORT, 'campaign_play_actor_due_set_capacity_invalid') END;
  SELECT CASE WHEN EXISTS (
    SELECT 1 FROM json_each(NEW.decisions_json) decision
    WHERE json_extract(decision.value, '$.disposition') = 'defer'
      AND json_extract(decision.value, '$.reason') NOT IN ('incapacitated', 'actor_capacity')
  ) THEN RAISE(ABORT, 'campaign_play_actor_due_set_defer_invalid') END;
...
BEGIN
  ...
  SELECT CASE WHEN NOT EXISTS (
    ...
    WHERE job.campaign_id = NEW.campaign_id AND job.actor_id = NEW.actor_id
      ...
      AND job.defer_reason = 'replan_capacity' AND job.completed_at = NEW.updated_at
      AND NEW.plan_id = OLD.plan_id
