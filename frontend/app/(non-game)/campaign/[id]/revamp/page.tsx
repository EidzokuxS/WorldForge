...
export default function RevampCampaignShellPage(props: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      .catch((loadFailure) => {
        if (!cancelled) {
          setLoadError(getErrorMessage(loadFailure, "Failed to load revamp campaign shell."));
        }
      })
      .finally(() => {
        if (!cancelled) {
  ...
  async function handleDescribe() {
    ...
    setActionError(null);
    try {
      const result = await parseRevampPlayerCharacter(campaignId, {
      ...
      setActionError(getErrorMessage(parseFailure, "Failed to create revamp player draft."));
    } finally {
  ...
  async function handleImport(file: File) {
    ...
    try {
      ...
      const result = await importRevampPlayerV2Card(
        campaignId,
        card,
        {
          importMode,
      ...
      setActionError(getErrorMessage(importFailure, "Failed to import revamp player draft."));
    } finally {
      setCharacterBusy("idle");
    }
  }
  ...
  async function handleSavePlayer() {
    ...
    setActionError(null);
    try {
      const result = await saveRevampPlayerCharacter(campaignId, {
        draft,
        source: draftSource,
      });
      setKernel(result.kernel);
    } catch (saveFailure) {
      setActionError(getErrorMessage(saveFailure, "Failed to save revamp player cast."));
    } finally {
      setCharacterBusy("idle");
    }
  }
  ...
  if (loadError || !campaign || !kernel) {
    return (
      <main className="mx-auto flex w-full max-w-3xl flex-col gap-4 px-6 py-10">
        <h1 className="font-serif text-3xl font-semibold text-[var(--fg)]">Revamp shell failed</h1>
  ...
  return (
    <main
      ...
      data-testid="revamp-campaign-shell"
    >
      <section className="grid gap-4 border-b border-[var(--line)] pb-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-xs uppercase tracking-[0.14em] text-[var(--fg-3)]">WorldForge Revamp</p>
      ...
      <section className="grid gap-4 md:grid-cols-[1fr_1.2fr]">
        <div className="grid gap-4">
          <article className="border border-[var(--line)] bg-[var(--bg-2)] p-5">
            ...
            <dl className="mt-4 grid gap-3 text-sm">
              ...
              <div>
                ...
                <dd className="text-[var(--fg)]" data-testid="revamp-kernel-phase">
                  {kernel.phase}
                </dd>
              </div>
              <div>
          ...
          <article
            ...
            data-testid="revamp-player-cast-panel"
          >
            <div className="flex items-center gap-2 text-sm font-semibold text-[var(--fg)]">
              <UserRound className="h-4 w-4 text-[var(--ember-1)]" />
              Player cast
            ...
            <div className="mt-4 flex flex-wrap gap-2" role="tablist" aria-label="Revamp player creation modes">
              <button
                type="button"
                role="tab"
            ...
            {characterMode === "describe" ? (
              <div className="mt-4 grid gap-3">
                <textarea
                  ...
                  aria-label="Revamp player concept"
            ...
            <label className="mt-4 grid gap-1 text-sm text-[var(--fg-2)]">
              ...
              <textarea
                ...
                aria-label="Revamp player override"
          ...
          <article className="border border-[var(--line)] bg-[var(--bg-2)] p-5">
            ...
            <p className="mt-3 text-sm leading-6 text-[var(--fg-2)]" data-testid="revamp-worldgen-boundary">
              Player cast writes to the Campaign Kernel. Full worldgen is parked on legacy routes.
            </p>
