...
3. Compile every active key/support/collective goal into a typed actor plan of three to eight causal steps and a deterministic schedule. Opening plans must carry ordinary actor work across several opportunities; Actor Replanner begins only when a plan is exhausted or accepted events make its next step stale. Every step is action by its owning actor alone: contact does not establish a response, later steps cannot assume another actor's consent or work, and movement cannot carry an uncommitted squad, companion, vehicle, or cargo.
