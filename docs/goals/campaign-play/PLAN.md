...
4. Build each actor frame from the latest committed mechanical version and that actor's profile, goals, placement, directed relations, durable knowledge, local routes, and known pressures.
...
**Acceptance evidence:** multi-actor fixtures prove every due actor receives a latest-version opportunity, earlier actor commits do not stale later actors, true stale work is rejected/rescheduled once, and the near-term off-screen consequence follows its typed exposure path.
...
```powershell
node --import tsx e2e/campaign-play/playtest-runner.ts --lane pristine-60 --run-config <fresh-premise-run.json>
node --import tsx e2e/campaign-play/playtest-runner.ts --lane pristine-60 --run-config <fresh-dna-research-run.json>
