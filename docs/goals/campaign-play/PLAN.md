...
A persistent sublocation is one directly perceivable scene, not a building or district containing offscreen rooms. Its stored `description` is a public-on-arrival surface and may contain only stable sensory or publicly obvious context; actor goals, relations, pressures, and protected events own secrets, concealed discoveries, private motives, and hidden causes. At this granularity, an actor's `present` placement means that the player can directly perceive and identify that actor when sharing the scene. Campaign Play may materialize a more precise persistent sublocation when a grounded local traversal succeeds. The accepted Campaign World definition stays immutable; the new scene and its two directed routes are receipt-bearing mechanical truth with code-derived identity, endpoints, travel cost, transition order, and persistence.

Current observations identify a person only through a code-owned performing-actor or observation-subject binding. Visibility may create a subject binding when a direct observation contains one exact canonical actor name and the authoritative exact-scene projection independently places that actor in `visibleActors`; the text selects an already visible identity but cannot create identity, presence, or performer authority. Before narration, code projects literal permitted and forbidden visible-actor names for each current observation index. A beat may name only the union of actors permitted by the observations it covers; names repeated by submitted text, prior prose, or an actorless observation summary do not create current attribution. Actorless sound, motion, traces, weather, and resemblance remain unattributed in narration; prose cannot promote them into actor identity or placement authority.

Mechanically sensitive event prose crosses an independent semantic review before Rulebook execution. The review rejects untyped changes to an actor's possession, obligation balance, payment, completed bargain, or route traversal authority. Attributed dialogue about the classification, filing, validity, disposal, or history of an untracked scene document remains presentation evidence when it changes none of those mechanics; vocabulary such as passage, bond, stamp, or clearance does not create a route rule outside an actual traversal claim.
...
- one Judge-owned `possessionEffectAuthority`, or explicit `none`; the typed authority names acquire/spend/transform, required or permitted enforcement, an existing visible possession when applicable, quantity, and the lowest result tier where it applies;
...
- Every deterministic or uncertain actionable result consumes at least one world minute. Judge compilation normalizes model-authored elapsed bounds to that floor; pure movement still uses the selected route's canonical travel cost, suggested waiting uses its fixed duration, and impossible or clarification-required results may remain at zero.
- Actor possessions are current Rulebook state. Acquisition and spending use typed quantity adjustments; prose-only possession claims, visible stock, cargo handling, work assignments, and general tool ownership carry no mechanical custody. Every opening-authored or replanned actor step carries mandatory typed `possessionOutcome` and `obligationOutcome`. Opening steps require `obligationOutcome: none`; replanned non-move steps may acquire a positive named possession, incur the acting actor's own debt, or pay that actor's own debt from its supplied possession, but one step cannot combine separate possession and obligation outcomes. Code derives owner, identities, scopes, transition order, receipts, events, and persistence while the authored sensory trace remains the visible summary. An acquisition emits one `AdjustActorPossessionCommand` instead of a duplicate prose event; move steps change neither possessions nor obligations, and spending/transform remain outside the actor possession lane. For player turns, the Judge emits `possessionEffectAuthority`: `required` compels exactly one matching acquire/spend/transform effect at or above its result threshold, while `permitted` allows zero or one matching effect when a targeted present actor may grant or refuse a transfer. Game Master compilation rejects every unmatched possession effect before Rulebook preflight. Authority also bounds semantic completion: an offer with `none` may negotiate terms, but no item changes hands and no bargained information, service, access, or other return is delivered. Calling an exchange paid, settled, square, fulfilled, or complete requires matching spend authority and an accepted possession transition in the same result. Possession quantities count indivisible Rulebook stack units; models cannot derive smaller units from numbers, durations, volumes, contents, or measures embedded in a possession name. A `Three days of travel food ×1` stack can be priced or spent only as one whole unit unless current state separately represents day-sized units. Filling or sealing a container inside a quantity-one plural set is a transform of the complete stack, never an acquire that leaves the container unchanged or a split into used, remaining, and newly acquired possessions. The model-authored result name must preserve the complete retained item or set and every material new content or state; summary cannot carry possession state omitted from that durable identity. Writing a usable record into a notebook, form, chart, ledger, or other retained object follows the same required-transform boundary. A model-authored `transform` compiles to an atomic source spend followed by a result acquire in the same Rulebook batch. Only the result command is projected as the public consequence, so the Narrator receives one authored summary while both state changes receive receipts.
- Actor obligations are separate current Rulebook state, not possessions, relations, cargo handling, or narrative claims. A player action may incur one definite copper debt in either direction between the human and one targeted visible nonplayer actor: an accepted charge makes the human the debtor, while completed definite unpaid work makes the nonplayer actor the debtor. Game Master compilation requires exactly one direction-matching `IncurActorObligationCommand`; code derives the deterministic debtor-creditor-unit identity, exact scopes, order, receipt, event, persistence, and public handle. Repeated incurrence for the same tuple accumulates principal and outstanding amount. During a player action, payment is available only for the human's own payable debt from one exact visible human-owned copper possession; a nonplayer cannot pay from undisclosed inventory. During actor work, the acting actor may incur only its own debt or pay only its own existing debt from its own persisted copper possession while co-located with the creditor. A matching `PayActorObligationCommand` atomically decreases the debtor's possession, increases the creditor's code-derived possession under the same key and name, and decreases outstanding debt without rewriting historical principal. A fully paid row remains in mechanical state with zero outstanding balance but leaves public state. Public projection labels direction explicitly as `payable` (`You owe <counterparty>`) or `receivable` (`<counterparty> owes you`) and preserves it through reload. A quoted price, warning, request, offer, promise, unrelated object transfer, `record_world_event`, or narrator prose never creates, reverses, increases, reduces, pays, or settles debt.
...
The opening path contains opening planner and narrator calls. Routine actor actions execute persisted typed plans. Actor replanning invokes a strict native-object planner only when a plan completes or its preconditions fail and must return at least three bounded causal steps, each with explicit possession and obligation outcomes. The frozen actor frame supplies only that actor's persisted possessions and obligations plus current co-located counterparties; the planner cannot invent another actor's inventory, debt, or payment. An independent schema-bound tool reviewer checks the proposed plan against the same accepted actor frame before persistence or execution. The reviewer returns only an accept/reject verdict: it cannot rewrite the plan, supply an outcome, retry the proposer, or fall back to backend prose. Its accepted trace must be the primary `tool_mode` result from the requested model, with no repair, full retry, text fallback, provider switch, or model switch. When either model returns a structurally or semantically invalid result, record the actor-replanner stage as `interrupted/model_contract_invalid`, terminalize that actor job as `deferred/replan_invalid`, advance its schedule by the persisted cadence, add agency debt, and continue settling the player turn. This path persists no proposal or replacement plan and performs no mechanical world mutation. Provider transport, stage-budget, lease, and persistence failures still interrupt the player turn and require explicit Resume. The proposer and reviewer remain one durable actor-replanner stage with combined model telemetry and a single worker epoch.

Every stage stores requested/actual model, strategy, token counts, duration, finish reason, schema outcome, and error code. Prompts contain bounded frames and opaque IDs. Model output has proposal authority only. Route travel cost is code-owned: Judge binds a pure move to the exact visible route cost and raises a compound traversal's elapsed bounds to at least that cost before Rulebook planning. For travel followed by contact, observation, or an attempt, the Judge preserves the post-travel action as the primary intent and carries the known route separately. When the accepted post-travel observation or attempt advances into a grounded local scene, Game Master must order the player route move before `enter_local_scene`. Compilation splits total elapsed time into the route's canonical cost and one positive local transition cost, then emits `advance_world_time -> move_actor -> advance_world_time -> materialized move_actor -> public event`. Rulebook receipts preserve that order and reload identity; model prose cannot merge the transitions or choose their costs.

Mechanically sensitive Game Master prose crosses an additional semantic authority boundary before Rulebook execution. Dialogue, interaction, possession- or obligation-affected events, route claims, and every pure possession transform are reviewed against their typed effects. A record event may negotiate, inspect, handle, or describe an untracked scene object, but it cannot itself acquire, spend, consume, transform, give, receive, or transfer a possession, incur or settle debt, complete payment, or deliver a bargained return. Those durable claims require the matching typed possession or obligation effect in the same accepted proposal. For a transform, the reviewer also compares the normalized intent, visible source possession, result name, and result summary; it rejects an unchanged name, an untracked split or remainder, and any name that omits material retained contents or state. The per-turn native schema and prompt expose only resource effect kinds permitted by the normalized Judge authority at the resolved result tier; an unavailable possession or obligation transition is absent from both authoring surfaces. A no-travel contact that targets or cites a route also requires `routeAccessClaims`: its relevant event declares route state, access requirement, and any intermediate location. Code first validates exact declarations and effects against the frozen Judge frame and current Rulebook state. One separate strict model call then reads only the sensitive summaries, validated claims, typed resource effects, source possession facts, normalized intent, and Judge authority and returns `accepted` or `rejected`; it cannot rewrite or repair the story. Rejection interrupts the durable Game Master stage before mutation, and explicit Resume creates a fresh proposal attempt. Acceptance persists a mechanical-authority review hash in the Game Master artifact and combines both calls' telemetry without classifying the reviewer as a retry. Earlier prose that conflicts with current typed authority is a continuity error to correct, not a source of mechanical truth. Narrator does not promote conflicting prose into a route requirement, possession, payment, debt, or settled exchange.

Suggested actions are exact player authorizations, not invitations for Judge or Game Master to fill in a decision. An ordinary move suggestion contains only the code-owned route destination (`Go to <destination>`); Narrator supplies no freeform route clue or purpose that could relocate evidence from another scene. Every available suggested `attempt` is bound to exactly one visible typed route; Visibility never publishes a generic route-less local attempt. Freeform interaction may traverse a room, corridor, threshold, floor, trail, or other local feature already established by the current source moment or a cited observation inside the current persistent location, or explicitly located inside the destination of a separately authorized visible route. Judge classifies that traversal as the primary `observe` or `attempt`; only the latter form also carries `movementRouteHandle`. When the accepted result physically changes the player's exact position, Game Master proposes the scene's public name and stable sensory description; code compiles that proposal into one typed move which atomically materializes the destination plus outbound and return routes, advances the player placement, and binds the local route cost to the accepted elapsed time remaining after any known-route travel. The Game Master frame includes code-owned local-scene authority: its forbidden-name list contains the current exact scene and any broad route destination, while its named-person roster is empty because the local transition moves only the player. The proposed local name must differ from every forbidden name so a mechanically distinct child cannot render as the current or broad parent scene, and a newly created scene cannot present a roster person who was never placed there. Code still rejects collisions rather than renaming a scene or repairing prose. The reverse route then owns the cost of retracing the local path, and routes outside the new exact scene remain unavailable until the player returns to their endpoint. When position does not change, Game Master may still commit an actorless local result without materializing topology. Player input alone cannot invent the feature, and entering another already known persistent location or visible route destination still requires one exact typed route. A suggestion that accepts, selects, orders, takes, or commits among mutually exclusive visible alternatives names one alternative; otherwise Narrator selects another intent. A click-to-submit suggestion is omitted when it requires a name, date, route, secret, answer, wording, disclosure choice, or other player-owned value absent from the packet; the player can supply that value through freeform input. A suggestion also preserves epistemic status: a claim made only by an NPC proves that the NPC made it, even when stated categorically, and does not become objective world state without independent packet corroboration. Questions, guesses, rumors, examples, possibilities, and conditionals carry the same boundary. The action keeps uncertainty actionable through attributed questions, requested checks, or investigation; possessive or definite wording requires packet evidence that the relation or thing exists. Freeform or suggested input that still omits a necessary alternative resolves as `clarification_required`. A route-less move is valid only in that non-actionable clarification state. Clarification changes no world state and Narrator presents exactly the Judge's question as one `action_handoff` beat.

Player history has its own semantic authority boundary. Literal CharacterRecord facts, chosen starting conditions, and the player's submitted actions may establish what the player previously experienced; an NPC statement, question, assumption, demand, motivation, or search target may not. Opening Planner cannot presuppose an unstated player experience, and Narrator cannot convert an NPC premise into viewpoint fact or a suggested action that adopts it. A chosen opening situation is descriptive, not route authority: only the typed visible route state may establish stopped, denied, gated, paid, or permission-bound passage.

Player motivation is decision pressure for the player, not authority to tailor the world. Opening Planner may expose a pressure that independently belongs to the selected location and actor plan, but it cannot choose a scene because that pressure resembles the motivation, invent an NPC need to match the player, broaden a literal craft or tool into another profession, or cast the player as uniquely expected, essential, or central. When the accepted world does not independently support an exact opportunity, the opening uses a grounded refusal, constraint, uncertain lead, or ordinary local interaction while the world's own pressure continues without becoming an assignment.

The admitted player intent owns its method and scope. Game Master preserves named trades, materials, tools, targets, exclusions, and refusals; a generic approach, observation, or wait cannot silently add an offer, transaction, profession, repair method, disclosure, promise, or commitment from earlier scene prose. Accepting an exchange authorizes only the stated acceptance: it cannot stand in for unspecified information, payment, transfer, promise, work, or other player-owned consideration, and missing content returns to the player before Game Master runs. Suggested-action priority follows the player's visible chosen direction rather than the world's highest stakes. Visibility projects a bounded chronological `playerHistory` from completed player-turn admissions and their accepted Judge public results; Narrator receives this beside the current `actionContext` rather than relying on model memory or prose reconstruction. Narrator `continuity` remains presentation history and never becomes Judge or Game Master visible-fact authority. Action admission exposes the packet's newly visible observations plus relevance-selected prior player observations at the current location; it does not flatten global journal entries into the current scene. A thread the player explicitly refused, corrected, or left remains resolved across later turns until a later player action deliberately re-enters it or a later accepted observation materially renews it; the original NPC need merely continuing is not renewal. An ordinary move to another location with no player-stated purpose leaves optional offers, tasks, search targets, and contact requests from the origin scene behind; Narrator cannot carry their names, leads, or follow-up questions into arrival suggestions without that explicit purpose or a new accepted observation at the destination.

An NPC's knowledge and the player's question establish subject matter, not disclosure authority. Game Master reveals a third party's identity, location, contact channel, or private case details, or asks the player to find or report on that person, only when current actor directives or visible facts supply a concrete speaker-side reason such as consent or public status, duty, established trust, reciprocal value already supplied or committed by the player, or immediate safety. Otherwise the NPC withholds, deflects, asks the player's purpose, or names a condition while keeping their own goals and relationships intact. This remains a model-owned social decision: code does not write dialogue, filter prose, retry the call, or supply a fallback response.

## Persistence and transaction design

### Accepted snapshot prerequisite

Campaign World acceptance stores the exact accepted review projection in `accepted_snapshot_json` together with `accepted_world_version` and `accepted_content_hash`. Accepted Review reads this immutable projection. Canonical location and route rows carry explicit definition authority: accepted rows remain the byte-stable Campaign World foundation, while receipt-bearing Campaign Play rows extend only the live mechanical topology.
...
Later current-contract migrations extend these same owners. `0035_campaign_play_actor_possessions.sql` adds actor-owned fungible quantities; it does not reuse the immutable CharacterRecord inventory or the displaced gameplay item/resource stores. `0036_campaign_play_character_inventory.sql` permits the protected character-bootstrap possession commands without a fictitious turn and widens only the internal bootstrap order bound; ordinary model, opening, player, and actor batches remain capped at sixteen commands. `0037_campaign_play_actor_obligations.sql` adds the current debtor-creditor-unit obligation balance and the `incur_actor_obligation` command/event/ref vocabulary. `0038_campaign_play_actor_obligation_payments.sql` permits a zero outstanding balance and adds the atomic possession-transfer/payment command and event; neither migration introduces a general economy or prose-derived debt. `0040_campaign_play_runtime_topology.sql` marks accepted versus Campaign Play location and route definitions and binds each live definition to the receipt and world version of the typed move that materialized it.
...
| `locations`, `location_edges` | immutable accepted definitions plus explicitly marked receipt-bearing Campaign Play scene and route definitions; live rows are never folded into accepted provenance |
...
| `campaign_play_actor_obligations` | positive historical copper principal and current nonnegative outstanding balance by deterministic debtor-creditor-unit key, with causal receipt and world version; zero-balance rows remain mechanical history and are omitted publicly |
...
- unique debtor-creditor-unit obligation key, positive principal, nonnegative outstanding amount not exceeding principal, and one current-state mutation per causal receipt;
...
Mechanical `worldHash` covers current world truth: world time; human actor identity and CharacterRecord digest; receipt-bearing runtime scene and route definitions; live route, actor condition, actor possession quantity, actor obligation principal and outstanding balance including settled zero rows, pressure state, placement, relation, and goal status. Only receipt-bearing character bootstrap, opening bootstrap, and Rulebook command batches advance `worldVersion/worldHash`.
...
6. Execute a valid typed step or persist a proposal based on that latest version. An actor move proposal names exactly one destination location. Code derives the single open directed route from the actor's step location to that destination and compiles both route and destination into the authoritative intent. No route, multiple matching routes, a blocked route, or a nonlocal destination invalidates the proposal instead of recording travel prose.
...
11. Increase agency debt when policy defers a due actor and record the reason. An invalid actor replan uses `replan_invalid`, preserves the invalid model-stage evidence, advances the schedule by its persisted cadence, and does not interrupt the already committed player action.
...
The cue describes the player's earned evidence and never names a hidden cause. The public narrator packet contains only current location, visible present actors, visible routes, directly sensed pressure, newly exposed observations and consequences, already known journal entries needed for continuity, elapsed time, opaque choice handles, and code-owned observation-subject bindings. When a newly visible consequence carries a typed performing actor, Narrator names that actor in the beat carrying the consequence before any required reply choice is presented; typed attribution cannot be reduced to agentless aftermath. A subject binding maps one new observation to the non-performing people named by its authoritative affected references and placed in the exposure scene by its event-time snapshot. It excludes the human actor and the attributed performer. The binding has presentation authority only: it prevents an unnamed target such as a hooded figure from being merged with the player, and cannot create a mechanical effect, world mutation, or actor identity absent from the accepted event. A visible actor carries display name, monogram, short public descriptor, and identity-derived visual accent; faction tags, internal actor IDs, and raw CharacterRecord fields stay absent. For a committed movement, Game Master receives a code-authoritative destination scene containing the public location surface and the names of actors with live `present` placement there. Arrival prose must preserve that roster without making a listed actor speak or act unless accepted effects establish the action.
...
3. Compile every active key/support/collective goal into a typed actor plan of three to eight causal steps and a deterministic schedule. Opening plans must carry ordinary actor work across several opportunities; Actor Replanner begins only when a plan is exhausted or accepted events make its next step stale. Every step is action by its owning actor alone: contact does not establish a response, later steps cannot assume another actor's consent or work, and movement cannot carry an uncommitted squad, companion, vehicle, or cargo. Every opening step explicitly declares `obligationOutcome: none`; runtime debt does not exist before bootstrap.
4. Purely validate proposed player placement, actor plan targets, reachability, local pressure, visible support actor, and every step's sequential location. A move uses exactly one directed route from the location established by prior steps; a non-move location target is the actor's location at that step.
...
**Acceptance evidence:** seeded fixtures cover user-selected and delegated starts, three-to-eight-step plan bounds, sequential directed movement, local non-move actions, unreachable/invalid targets, collective plans, the near-term hidden consequence, deterministic artifact hashes, and zero repository calls. Target-player facts answer where they are, what presses now, who is present, and what they can try, while excluding distant actor identity, hidden goals, and global cast/state dumps. A fresh rendered opening must persist three to eight steps for every initial actor plan and reach early scheduled work without an immediate one-step `plan_exhausted` conversion.
...
8. Require every newly authored actor step to declare `possessionOutcome` and `obligationOutcome`. Compile a positive named acquisition into exactly one own-actor `adjust_actor_possession` command with a durable receipt and public authored summary; compile both `none` outcomes through the existing world-event lane. Add no persisted-plan default or compatibility adapter.
...
10. Compile an actor `incur` outcome only with the acting actor as debtor and a supplied co-located creditor. Compile `pay` only for that actor's supplied outstanding obligation, its own persisted copper possession, and a co-located creditor. Reject remote creditors, borrowed inventory, reversed parties, and prose-only settlement before proposal persistence or Rulebook execution.
...
**Acceptance evidence:** multi-actor fixtures prove every due actor receives a latest-version opportunity, earlier actor commits do not stale later actors, true stale work is rejected/rescheduled once, the near-term off-screen consequence follows its typed exposure path, and a visible actor acquisition correlates one typed proposal with one possession transition, receipt, persisted quantity, and non-generic presentation. Directed-obligation fixtures prove an actor can incur and later pay only its own debt from its own persisted possession while co-located with the creditor, with proposal, receipt, presentation, persistence, and reload identity correlated. A clean rendered campaign separately proves model-authored `none` outcomes create neither possessions nor obligations and that a visible actor replan survives reload. A retained Black Rain negative turn proves that an offer and permission do not authorize an actor plan to settle the player's unperformed repair; the repaired replay must keep the actor's plan and visible trace within Pia's own action unless a later accepted player event establishes the repair.
...
2. Every persistent player placement change uses one visible concrete-to-concrete route and the existing receipt-bearing `move_actor` command. A grounded local traversal may change the presented scene relation inside the current persistent location through an actorless event, but it cannot change placement, enter an unmodeled establishment, or reach a visible route destination. This preserves the Rulebook movement contract without turning one persistent location into an indivisible room.
