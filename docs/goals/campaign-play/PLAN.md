...
- Actor obligations are separate current Rulebook state, not possessions, relations, cargo handling, or narrative claims. The Judge may require one definite copper debt from the player to a visible nonplayer creditor at or above a named result tier. Game Master compilation then requires exactly one matching `IncurActorObligationCommand`; code derives the deterministic debtor-creditor-unit identity, exact scopes, order, receipt, event, persistence, and public handle. Repeated incurrence for the same tuple accumulates principal and outstanding amount. A quoted price, warning, request, possible charge, `record_world_event`, or narrator prose never creates, increases, reduces, pays, or settles debt. Payment and settlement require a later typed Rulebook contract and are outside the incurrence slice.
...
| `campaign_play_actor_obligations` | current positive copper principal and outstanding balance by deterministic debtor-creditor-unit key, with causal receipt and world version |
...
- unique debtor-creditor-unit obligation key, positive principal/outstanding amount, and one current-state mutation per causal receipt;
...
Mechanical `worldHash` covers current world truth: world time; human actor identity and CharacterRecord digest; live route, actor condition, actor possession quantity, actor obligation balance, pressure state, placement, relation, and goal status. Only receipt-bearing character bootstrap, opening bootstrap, and Rulebook command batches advance `worldVersion/worldHash`.
