...
Route topology and access claims in Game Master prose cross an additional semantic authority boundary before Rulebook execution. A no-travel contact that targets or cites a route requires `routeAccessClaims` on every proposed world event: its relevant dialogue or interaction declares the route state, access requirement, and any intermediate location, while its other events use an empty array. A general question about passage, clearance, stamping, permits, tolls, or fees cites every currently visible route through the Judge rather than inventing one semantic target; the Game Master must return exactly one claim for every cited route. Events outside that route-contact boundary omit the field. Code first validates the exact declaration set against the frozen Judge frame and current Rulebook route state. A separate strict model call then reads only the proposed summaries and validated claims and returns `accepted` or `rejected`; it cannot rewrite or repair the story. Rejection interrupts the durable Game Master stage before mutation, and explicit Resume creates a fresh proposal attempt. Acceptance persists a review hash in the Game Master artifact and combines both calls' token, cost, and duration telemetry without classifying the reviewer as a retry. Earlier dialogue that conflicts with current typed route authority is a continuity error to correct, not a source of mechanical truth. Narrator does not turn that conflicting prose into another stamping, clearance, permit, toll, or fee suggestion when typed state leaves every visible route open and no obligation supplies one.
